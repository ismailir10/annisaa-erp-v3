...
- Ship fix iteration 1 rebase gate: rebased cleanly onto `origin/staging` (`f11cee33`); `npm run build` passed including TypeScript, and `npx vitest run` passed (259 files passed / 2 skipped; 2554 tests passed / 42 todo).
- Preview verification: Vercel deployment `dpl_3DcvkMHLNxo6E9k4iTxsvZrqiD9F` reached `READY`; signed-in teacher smoke passed at 320×720 and 375×812 with zero horizontal overflow. Dashboard retained five navigation items; the leave flow exposed the successful empty state and labeled `Jenis Cuti`, `Tanggal Mulai`, `Tanggal Selesai`, and `Alasan` controls; class attendance exposed labeled class/date controls and stable roster rows; weekly day selection honored ArrowRight and End; logout exposed `Keluar dari akun` and canceled cleanly.
- Preview finding triage: one React hydration warning appeared only during the Google OAuth redirect into the first dashboard tab; a fresh authenticated dashboard load reproduced zero console errors. Classified non-blocking and rejected as transient navigation state, with no product or branch regression reproduced.
