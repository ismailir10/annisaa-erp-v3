...
| **curriculum** *(in flight — C1 of 11)* | PROMES spine: Semester → Theme → SubTheme → Week, LearningObjective → AchievementIndicator → IndicatorThemeLink. Cycle 1 (this PR) lands schema + Semester/Theme/SubTheme/Week admin CRUD; PROMES xlsx import is C2, Objective/IKTP CRUD is C3, AssessmentEntry is C4. Permissions: `curriculum.read` (TEACHER + SCHOOL_ADMIN + SUPER_ADMIN), `curriculum.write` (SUPER_ADMIN). See [design spec](docs/superpowers/specs/2026-05-12-curriculum-penilaian-raport-design.md). |
