...
| Date | Decision | Why |
|---|---|---|
| 2026-05-08 | v2 addresses + idn region chain — migration `10_addresses` + `Address` EntityPolicy + 4 region GET routes (`/api/regions/{provinces,regencies,districts,villages}`); RLS 33/33; `Household.addressId` composite FK `SET NULL ("addressId")` per scaffold.md §6 | Closes §4.1 deferred Address model; chain-validity app-layer Zod BPS prefix + DB compound FK; orphan parent → 200 empty (Postel's law) — see [cycle](docs/cycles/2026-05-08-p2-addresses-idn-chain.md) |
