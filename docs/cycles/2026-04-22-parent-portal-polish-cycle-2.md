...
- [ ] **T3 — Teacher text-size + PortalTabs sweep. Dep: T1.**
...
- [ ] **T5 — Teacher header adopts `PortalHeader`. Dep: T2.**
...
- [ ] **T6 — Parent `child-selector-tabs` uses `leading` avatar. Dep: T1.**
...
- [ ] **T7 — Parent `error.tsx`. Independent.**
...
- [ ] **T9 — Parent Catatan write dialog + edit + delete. Dep: T1 (none blocking, but groups with T1's test infra).**
...
- [ ] **T12 — Parent Kehadiran — API pagination + server-paginated DataTable. Independent (large task).**
