...
- Preview-verify iteration 1 (https://annisaa-erp-v3-git-feat-siswa-4e02d9-ismails-projects-196d40d3.vercel.app): flows=[students list Wali column, guardians list Siswa column, student → wali → child → Saudara → student round trip, Tambah Wali three steps], blockers=1, minors=0. Signed in as the admin account per the role-scoped table. No console errors on any page.
  - Confirmed working: Wali cells are real `<a href="/admin/guardians/...">`; guardians list renders `2 siswa · Bilal Hakim, Hafizh Umar Ramadhan` for the one multi-child wali in staging; the student page shows the wali link plus a **SAUDARA** chip; every hop of the round trip navigates. The picker returns `Nama · telepon · email · N anak terdaftar` and **Tautkan Wali** stays disabled until a parent is selected.
  - **Blocker — blank Email rejected on create.** "Tambah Wali Baru" with the Email box left empty toasted *"Email tidak valid"* and 400'd before the duplicate guard ran. Pre-existing (`origin/staging` has the same `z.string().email()` rule, and `EMPTY_GUARDIAN_FORM.email` is `""`), but it makes this cycle's create path unusable for exactly the families it targets — an emailless parent is the one that used to get a fresh Parent row per child, so the candidate warning was unreachable for them. Fixed in iteration 2 via `lib/validations/optional-email.ts`, applied to `createGuardianSchema`, `updateGuardianSchema` and `updateParentSchema`; the last of those also unblocks saving bio on an emailless wali through the T8 route change.
- Preview-verify **converged on iteration 2 (clean)**: 2 iterations, 1 fix commit (`03e0a37b`), final preview https://annisaa-erp-v3-git-feat-siswa-4e02d9-ismails-projects-196d40d3.vercel.app. Blockers 0, minors 2.
  - Re-walked the failing flow: "Tambah Wali Baru" with the Email box blank and Nama typed as `dewi   siregar` (lowercase, triple-spaced) now reaches the **Periksa Data Wali** step and matches the stored `Dewi Siregar`, labelled *"Cocok pada nama yang sama"*, with **Tautkan** and **Tetap Buat Baru**. `POST .../guardians` returned 409 as intended; nothing was written. This is the JS-normalisation fix from the T1 review pass confirmed against real Postgres data rather than a mock.
  - `PUT /api/parents/[id]` → 200 on the guardian detail page, with the re-fetch following it — T8's route change verified end to end.
  - Minor: the candidate row's detail line truncates mid-word at dialog width (`… · 1 anak terdaft…`). Legible enough to identify the wali; not worth a fix commit on the ship path.
  - Minor: several `?_rsc=` prefetch requests and `/.well-known/vercel/jwe` return 503 on the preview. All are on routes this cycle does not touch (`semesters`, `raport/templates`, `student-journal`) and are Vercel preview prefetch/cold-start artifacts — every page actually navigated returned 200, and no request under `/api/students`, `/api/parents` or `/api/guardians` returned ≥500.
