...
| **students** | Student lifecycle — auth-proxied photo upload, guardians (one parent record shared across siblings; Tambah Wali searches existing wali first, with an overridable duplicate warning on a matching name, phone, NIK or email), enrollments (one school plus one concurrent day-care placement per year; age bands advisory, overridden with an audited reason), admissions (`/daftar` inquiry → emailed token → enrollment form → convert), CSV export. Detail is a single-scroll dossier — anchor nav, collapsible sections, summary rail, NIK/KK masked |
