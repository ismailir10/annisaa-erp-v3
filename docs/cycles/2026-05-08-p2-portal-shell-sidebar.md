...
- [x] **T4 — SELF-scope canary widening (Guardian.update) + footgun-mitigation meta-test.**
...
- [x] **T5 — Playwright shell + cross-portal redirect (3 specs).**
  - `e2e/admin/portal-shell.spec.ts` (NEW) — per AC7. Cross-portal redirect: `await page.goto('/parent'); await page.waitForURL('/', { timeout: 5000 });` (SD5 sequential pattern — corrected per spec-time review SR2; navigates to `/parent` root rather than `/parent/foo` because Next.js 16 may bypass the layout chain on a hard-404 leaf, masking the assertPortalAccess redirect — per spec-time review T5-#2).
...
- [x] **T6 — Cycle doc Implementation + Verification + Ship Notes; README ADR row.** *(README ADR row landed in T1 commit; Ship Notes filled here.)*
...
- T2 gates: `npm run build` succeeds with new layouts mounted (3 layouts wrap their portal route trees; existing pages render unchanged inside `<main className="flex-1">`). `npx vitest run lib/portal` 10/10 (sidebar tests untouched at 6 — total still 16). Drift-test scope tightened in source comments per SR-T2-#1.
- Post-PR-open lint fix: `components/portal/sidebar-item.tsx` switched to `React.createElement(resolveIcon(name), props)` — CI lint flagged `const Icon = resolveIcon(name); <Icon ... />` under the `react-hooks/static-components` rule (treats the binding as a fresh component each render). Vitest pattern unaffected; rendered DOM identical.
- End-of-cycle gates (after T5): `npx prisma generate` clean. `npm run build` 200ms full route table includes `/admin`, `/teacher`, `/parent` (was admin only). `npx vitest run` 1065 passed | 4 skipped (was 1044 from p2-entity-actions tip — delta +21: 16 portal/sidebar/guard + 7 SELF/scaffold − 2 inverted Guardian-update FORBIDDEN cases; AC8 budgeted +13–15, actual was higher because spec-time review T4 mitigations added the meta-test + extra assertScope cases). `bash scripts/verify-rls-coverage.sh` 32/32 (no schema change confirms SD2). `bash scripts/verify-api-auth.sh` 5/5 (no new API routes). `bash scripts/verify-pii-annotations.sh` 5/5. `npm run scaffold:check` 5/5. Playwright local: 2/3 new specs pass against local DB (admin + parent — teacher requires staging reseed for `homeroom_teacher` demo user added in T5; CI auto-reseeds before Playwright job runs per p2-entity-actions PR #202 precedent). Existing `e2e/admin/students.spec.ts` Household-seed assertion is a pre-existing local-only seed dependency unaffected by this cycle.
- T5: 3 new Playwright specs at `e2e/{admin,teacher,parent}/portal-shell.spec.ts` covering sidebar visibility (`<nav aria-label="Portal navigation">`), Akademik group heading + active-route highlight (`aria-current="page"`), all teacher/parent flat-list items per spec-time review SR-T5-#1, and cross-portal redirect (each portal navigates to a sibling portal's ROOT and `waitForURL("/")` confirms `redirect("/")` from the layout guard — sequential pattern per SD5; navigates to root not `/foo` because Next.js 16 may bypass the layout chain on a hard-404 leaf, per SR-T5-#2). Stub `app/teacher/page.tsx` + `app/parent/page.tsx` (NEW, ~17 lines each) added so those routes exist as real pages — required for the layout guard to fire reliably under Playwright. `prisma/seed/08-demo-users.ts` extended with `teacher@demo.local` (roleCode `homeroom_teacher`) — `/api/demo/login?role=teacher` previously 500'd with `no_seed_user` since seed only carried admin + parent. CI re-seeds before Playwright runs; local Playwright validates admin + parent locally (teacher requires staging reseed). Existing `e2e/admin/students.spec.ts` Household-seed assertion is a pre-existing seed-dependency unaffected by this cycle.
- T4: `lib/scaffold/server-action.ts` — `assertScope` now returns the resolved `ScopeGrant` (was `void`); writes-gate widened from `scope !== "ALL"` → `scope !== "ALL" && scope !== "SELF"`. ⚠ SELF-write contract documented at top + above the `ScaffoldScope` union. `lib/entities/guardian/policy.ts` — `{ role: 'parent', scope: 'SELF' }` added to update grants. `lib/guardians/actions/update.ts` — captures grant from `assertScope` (fixes spec-review SR-T4-#1 duplicate-grant footgun); SELF row-level predicate via `restrictToSelf` flag; defense-in-depth `id_tenantId` compound-unique on inner `tx.guardian.update` (SR-T4-#3). Tests: `lib/scaffold/__tests__/self-write-contract.test.ts` (NEW, 1 case static text-scan over POLICY_BY_RESOURCE), `lib/scaffold/__tests__/server-action.test.ts` (NEW, 2 cases — SELF-write passes, OWN_STUDENT-write fails-closed), `lib/guardians/actions/__tests__/actions.test.ts` extended (parent-update test inverted from FORBIDDEN to SELF-pass; +3 cases for SELF-update on own row, SELF-update on wrong row → NOT_FOUND, admin ALL regression). Spec-time review T4-#2 surfaced: parent demo seed lacks Guardian.userId link → real-tenant smoke needs invitation acceptance flow (precondition documented inline in update.ts; deferred to `p2-portal-write-widening`). Vitest: 1065 passed (+7 from T1+T2 baseline 1058 → 4 new + 4 actions extension + 1 sanity + meta-test self-check; one prior parent-update FORBIDDEN test inverted, no net delete).

## Ship Notes

- **Migrations:** none (no Prisma schema diff — SD2 chose existing `SELF` enum literal over a new `OWN_GUARDIAN` migration).
- **Env vars:** none.
- **Reseed required on staging:** YES (per spec-time review T5). `prisma/seed/08-demo-users.ts` adds `teacher@demo.local` (roleCode `homeroom_teacher`). `/api/demo/login?role=teacher` 500s with `no_seed_user` until the seed runs. After merge: `bash scripts/reseed-staging.sh` (existing precedent — same as `p2-entity-actions` cycle's Household additions). Idempotent — safe to re-run. Note: real-tenant parents acquire `Guardian.userId` only via invitation acceptance flow; demo seed does NOT create a Guardian row linked to the demo parent's `userId`, so the SELF-update canary (T4) cannot be smoke-tested via demo login until a follow-up cycle wires the demo parent Guardian seed (deferred to `p2-portal-write-widening` per inline note in `lib/guardians/actions/update.ts:42`).
- **Manual smoke on preview URL:**
  1. `POST /api/demo/login?role=admin` → 200 + cookie.
  2. Navigate to `/admin/akademik/siswa` — assert sidebar visible (left rail), 5 group headings (Akademik / Operasi / Keuangan / Identitas / Sistem), Siswa link highlighted with `aria-current="page"`.
  3. Click the collapse toggle in the rail header — sidebar shrinks to 64px (icon-only); cookie `portal-sidebar-collapsed=1` written. Reload — sidebar still collapsed (SSR-no-flicker).
  4. Resize viewport <768px (Chrome devtools) — desktop rail hides, hamburger trigger appears top-left. Click — drawer opens. Press Esc — drawer closes.
  5. Navigate to `/parent` (still admin-cookied) — observe redirect to `/`.
  6. `POST /api/demo/login?role=parent` → 200 + cookie. Navigate to `/parent` — Beranda stub renders inside the parent sidebar shell (4 items: Beranda + 3 disabled stubs).
  7. `POST /api/demo/login?role=teacher` → 200 + cookie. Navigate to `/teacher` — Beranda stub renders inside the teacher sidebar shell (4 items: Beranda + 3 disabled stubs).
- **Rollback plan:** revert PR — sidebar disappears (3 layouts revert to non-existent → portal route trees render bare children as before); Guardian.update SELF grant reverts (parents lose update access, returns to FORBIDDEN; admin/principal/kadiv/admission_officer ALL grants unaffected); meta-test removes itself (no SELF-write grants remain to verify). The teacher demo seed stays harmless if reverted-after-reseed (extra User row, no consumer breaks). §18A row reverts to `next` if `/ship` flip already ran — Step 3 idempotency handles re-flip on re-merge.
