...
// Returns villages for a given districtId. Tenant-agnostic (global reference data).
...
const querySchema = z.object({
  districtId: z.string().regex(/^\d{6}$/, "invalid_district_id"),
...
export async function GET(req: NextRequest) {
  ...
  const districtIdRaw = url.searchParams.get("districtId");
  if (!districtIdRaw) {
    return NextResponse.json({ error: "missing_parent_id", field: "districtId" }, { status: 400 });
  }
  ...
  const { districtId, page, pageSize } = parsed.data;
