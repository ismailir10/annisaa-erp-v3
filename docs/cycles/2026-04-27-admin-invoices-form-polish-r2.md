...
- **End-of-cycle Playwright re-run (post-T5, pre-/ship):** operator freed port 3000 (killed dev server). `npx playwright test` → **21 passed (18.4m)** chromium. Suite covers admin auth, monthly attendance grid, payroll list, settings pages, employee dialog, assessment-template redirect, program/section/enrollment deactivate, student-attendance override+void, payroll detail/edit (409 on non-DRAFT), teaching-assignment role edit, admission state machine, invoice void, Buku Penghubung admin, bulk+manual+retry tagihan flows, design-system reference page, teacher salary slips. **No regression from T1 primitive changes or T2/T3/T4/T5 dialog edits.**
