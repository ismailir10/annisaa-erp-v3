...
All four portals share the same brand chrome — An Nisaa' logo + "Talib" wordmark + "by An Nisaa' Sekolahku" sub-label. Login screen carries the tagline "Sahabat belajar anak — kehadiran, jurnal, tagihan dalam satu pintu." (Bu Sari voice, see [`.claude/standards/voice.md`](./.claude/standards/voice.md)). Outbound emails (salary slips today; invoices via Xendit) use sender display `RESEND_FROM_EMAIL="Talib by An Nisaa' <noreply@annisaasekolahku.com>"` — Resend DKIM/SPF/DMARC verified for `annisaasekolahku.com`.

**Public surfaces** (no auth required):
- `/login` — Google OAuth entry (Supabase PKCE; lands at `/admin` on success)
- `/daftar?tenant=<slug>` — Public admission form (5-step: Anak → Orang Tua → Alamat → Program → Tinjau); creates `Admission` row in `SUBMITTED` state and emails the parent a tracking code
- `/legal/terms`, `/legal/privacy` — Indonesian PDP boilerplate, footer-linked from login
...
| Date | Decision | Why |
|---|---|---|
| 2026-05-10 | v2 scaffold list CRUD parity — `<ScaffoldListPage>` adds Add CTA + filter row + action column + row-click + empty-state CTA + total-count subtitle via two client islands (`<ScaffoldListPageToolbar>`, `<ScaffoldListRow>`). New optional `EntityDef.rowActions?` + `createDisabled?` fields | Pre-emptive scaffold sweep before `p2-mpls-minimal-admin` so MPLS list inherits parity. Server-action `assertScope` is sole gate (v1) — see [cycle](docs/cycles/2026-05-10-p2-scaffold-list-crud-parity.md) |
| 2026-05-09 | v2 admission funnel schema (split 1/2) — migration `11_admission` (Admission + InitialAssessment + MplsCohort + MplsMember + MplsAttendance + 3 enums + 2 column-list `SET NULL` composite FKs on Admission); `lib/admission/state-machine.ts` (8-state / 12 legal edges); RLS 38/38, audit-pii 10/10 (+5 on Admission). UI half = `p2-admission-funnel-ui` | Pre-emptive §18.2 cap split; 8-state funnel scope-locks §10A.4 13-state finance taxonomy (P3 extends `ALTER TYPE`) — see [cycle](docs/cycles/2026-05-09-p2-admission-funnel-schema.md) |
| 2026-05-08 | v2 addresses + idn region chain — migration `10_addresses` + `Address` EntityPolicy + region GET routes + `createAddress`/`updateAddress` server actions + `<AddressChainField>` cascading-Select × 4 + Household edit wire-in; RLS 33/33 | Closes §4.1 deferred Address model; chain-validity Zod BPS prefix (full create) + DB compound FK (partial update safety net) — see [cycle](docs/cycles/2026-05-08-p2-addresses-idn-chain.md) |
| 2026-05-08 | v2 portal shell + sidebar — `assertPortalAccess()` layout role guard (mismatch → `redirect("/")`); `Sidebar` Client w/ Shadcn `Sheet` mobile + 240px rail + collapse-cookie; `NAV_BY_PORTAL` IA per §10A.1 (labels from entity registry); canary `Guardian.update` SELF grant for `parent` w/ row-level `userId` predicate; meta-test enforces SELF-write pairing | Lifts the strict-ALL-writes stopgap from p2-scaffold-pages by surfacing portal-role visibility + ONE SELF round-trip — see [cycle](docs/cycles/2026-05-08-p2-portal-shell-sidebar.md) |
...
| 2026-05-06 | v2 scaffold registries — `lib/entities/` per-entity `{schema,entity,policy}.ts` + `_types.ts` `EntityPolicy` + barrel; 5→6 entities (Address added in `p2-addresses-idn-chain` T2). `scaffold.md` codifies conventions; dataFetcher admin tenant-scoped | First scaffold-engine consumers; unblocks p2-scaffold-pages + p2-scaffold-canary — see [cycle](docs/cycles/2026-05-06-p2-scaffold-registries.md) |
