...
- Post-lint CI re-run surfaced `scripts/verify-api-auth.sh` blocking on `app/api/admission/submit/route.ts` (cycle 1.1's public form endpoint shipped via PR #240) — missing `// @public` sentinel on line 1. Added; route is intentionally unauthenticated with a 5/min/IP rate limit. Same baseline class.
