...
| Date | Decision | Why |
|---|---|---|
| 2026-05-14 | Curriculum C5 (in flight) — sentra (CENTER) daily assessment on top of C4's `AssessmentEntry`; `assessmentEntryCenterSessionSchema` validator (≤80 entries/session, empty-allowed) | Reuses C4 schema with `source = CENTER` discriminator so 8 sentra rooms feed the same per-student data — see [cycle](docs/cycles/2026-05-14-curriculum-c5-center-assessments.md) |
