...
| Date | Decision | Why |
|---|---|---|
| 2026-05-14 | Curriculum C6 (in flight) — parent perkembangan rollup; `assessments.read` granted to GUARDIAN; `getParentChildById` gates per-child reads; `loadStudentPerkembangan` aggregates per-element counts + latest-pekan preview; `GET /api/parent/perkembangan/[studentId]` | Closes parent-visibility loop on C4+C5 entries (per-element CONSISTENT/EMERGING/NEEDS_REINFORCEMENT counts over active semester) — see [cycle](docs/cycles/2026-05-14-curriculum-c6-parent-perkembangan.md) |
