...
- C3. Restore `xenditSessionId` fallback in `handleSessionCompleted` + `handleSessionExpired`. On `findUnique({ id: reference_id })` miss, fall back to `findFirst({ where: { xenditSessionId: payload.payment_session_id ?? payload.id } })`. **No `tenantId` filter** — webhooks are tenant-agnostic at the Xendit-account level (single Xendit merchant per ERP install) and `xenditSessionId` is unique enough to resolve the row. When multi-tenant Phase 2 lands, the fallback will need a `tenantId` derived from `xenditSessionId → invoice.tenantId` join. Log line on fallback hit: `{ level: "warn", reason: "ref_id_miss_session_fallback", refId, sessionId, paymentId, invoiceId }`. Resurrect the two unit tests originally added in PR #136.
...
- Task 4 — Concurrency limiter + per-student results + BatchProgressCard polish. NEW: `lib/finance/concurrency-limit.ts` (10-LOC inline semaphore — no `p-limit` dep), `lib/finance/__tests__/concurrency-limit.test.ts` (10 tests). MODIFIED: `app/api/invoices/generate/batch/route.ts` + `lib/finance/xendit-retry.ts` (each instantiates `runLimit = limit(5)` per request, wraps Xendit fan-out so peak concurrency is capped — protects Vercel function timeout under Xendit latency spikes), both endpoints now return `results[]` carrying `studentName` + `error` per row. `lib/finance/run-bulk-generate.ts` + `lib/finance/run-bulk-retry.ts` extended with optional `signal: AbortSignal` (pre-chunk abort check short-circuits to `phase: "aborted"`) + `failures: FailureRow[]` accumulator on snapshot. `components/admin/invoices/batch-progress-card.tsx` adds `onCancel` prop wiring "Batalkan" ghost button, `beforeunload` listener with exact spec copy `"Pembuatan tagihan sedang berjalan. Yakin keluar?"`, collapsible `<details>` "Lihat detail (N gagal)" listing failures (read-only per B4 v1). `app/admin/invoices/page.tsx` wires AbortController refs into both orchestrators. Code-reviewer pass: 1 critical bug fixed inline — `runBulkGenerate` three-strike abort path was returning `{phase: "aborted"}` WITHOUT calling `onProgress`, leaving the React `progress` state frozen at the last `running` snapshot (stuck spinner + Batalkan button forever). Now emits `onProgress` with `phase: "aborted"` + `lastError` before returning, mirroring the correct `runBulkRetry` pattern.
- Task 5 — Webhook two-phase + hardening. MODIFIED: `prisma/schema.prisma` (WebhookEvent.status comment → `RECEIVED | PROCESSED | ERROR | IGNORED`, dropped DELETE-on-transient wording, replaced with "ERROR rows retained for admin audit; provider retries deduplicate via eventId @unique"), `app/api/xendit/webhook/route.ts` (full rewrite: Phase 1 INSERT always commits, P2002→200 short-circuit; Phase 2 errors mark row ERROR + return 200 ALWAYS — no DELETE-on-transient; xenditSessionId fallback restored when reference_id misses; amount-mismatch guard refuses missing/zero amount; soft-revert payment_session.expired to PENDING_PAYMENT_LINK instead of destructive CANCELLED; new markError helper). MODIFIED: `app/api/__tests__/xendit-webhook.test.ts` (file header rewritten with T5 contract, 4 prior tests updated for new behavior, 4 new T5 tests: MISSING_AMOUNT path, OVERPAYMENT_FLAGGED path, xenditSessionId fallback with real production payload, expired-on-PAID ignored). Code-reviewer pass: 3 fixes applied — (a) overpayment flag computation moved INSIDE the tx using `fresh.totalPaid` instead of pre-tx snapshot — closes a race window where two concurrent webhooks with different paymentIds could both miss the flag; (b) test file header comment updated to reflect new T5 contract (was still describing old DELETE+500 path); (c) spec C3 amended to explicitly note `tenantId` is intentionally omitted from the fallback (single-Xendit-merchant assumption; multi-tenant Phase 2 will need a tenantId-derivation step).
...
- Task 4: gates passed. `npm run build` → green. `npx vitest run` → 633 passed, 42 todo (+17: 10 concurrency-limit, +1 batch route assertion, +1 retry studentName, +5 abort/cancellation in orchestrators).
- Task 5: gates passed. `npm run build` → green. `npx vitest run` → 637 passed, 42 todo (+4 T5 hardening: missing_amount, overpayment_flagged, xenditSessionId fallback w/ real production payload, expired-on-paid ignored). 4 prior webhook tests rewritten to match T5 contract.
...
- Task 6: gates passed. `npm run build` → green. `npx vitest run` → 644 passed, 42 todo (+7 cron: missing-secret 500, wrong-bearer 401, missing-auth 401, non-vercel-UA 403, missing-UA 403, happy-path 200 with counts, SQL template inspection).
