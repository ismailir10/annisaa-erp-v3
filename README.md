...
| Date | Decision | Why |
|---|---|---|
| 2026-05-14 | Curriculum C4 (in flight) — `AssessmentEntry` + `AssessmentSource` (HOMEROOM\|CENTER) + `LearningCenter` (8 sentra) Prisma additions; `lib/validations/assessment-entry.ts` discriminator schema; `lib/curriculum/{homeroom,week-resolver}.ts` helpers; `assessments.read`+`assessments.write` perm keys granted to TEACHER | First write path against curriculum indicators. English-only code identifiers (Indonesian only in UI copy) per CTO directive — see [cycle](docs/cycles/2026-05-14-curriculum-c4-weekly-assessments.md) |
