...
| Date | Decision | Why |
|---|---|---|
| 2026-05-11 | Owner email driven by `OWNER_EMAIL` env (default `owner@example.test`); `prisma/data/students.ts` marked synthetic-only; migration SQL + `docs/cycles/archive/**` left as historical record | Public-repo prep: scrub personal owner address from `prisma/seed.ts` + `scripts/reseed/*` so no real PII at source tip. Set `OWNER_EMAIL` on Vercel staging/prod env before deploy — see [cycle](docs/cycles/2026-05-11-public-repo-prep.md) |
