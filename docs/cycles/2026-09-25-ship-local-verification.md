...
- No application API, schema or runtime behavior changes in this workflow PR.
...
- Source runtime baseline: `e630a23` (`origin/staging` at checkout); the PR changes only `.claude/skills/ship/SKILL.md`, `CLAUDE.md`, and this cycle document. Build-generated `next-env.d.ts` was restored and is excluded.
...
- Scenario review covers non-auth UI, auth callback/session dependency, dependency-only, pure docs, unavailable signed-in profile, failed local checks and moved PR heads. Required check conclusions and expected-head merge conditions remain mandatory.
...
- Playwright and interactive browser verification: skipped for this docs-only workflow change; no application, dependency, build or runtime configuration files changed. Required CI remains a merge gate.
