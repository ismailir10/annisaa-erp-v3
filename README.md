...
| Date | Decision | Why |
|---|---|---|
| 2026-05-12 | Curriculum C2 — PROMES xlsx import pipeline: `lib/curriculum/promes-parser.ts` (exceljs, alias-table substring match) + `POST /api/admin/curriculum/import-promes` (multipart, preview/commit, prisma.$transaction + audit, 409 on `(tenantId, semesterId, ageGroup, element, number)` collision, 413 row-count cap) + admin import page | C2 of 11 — PROMES xlsx is the canonical TP+IKTP seed path per design §6.1. Switched parser dep `xlsx` → `exceljs` over CVE-2023-30533. Conflict policy locked hard-reject (no overwrite toggle in C2; C3 CRUD UI handles surgical edits) — see [cycle](docs/cycles/2026-05-12-curriculum-promes-import.md) |
