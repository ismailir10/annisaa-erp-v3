...
- T11 follow-up (commit 1) — CI typecheck failed on first push (PR #184); typecheck is stricter than local `tsc --noEmit` because it runs after `npx prisma generate` against the workspace TypeScript config. Two fixes: (1) add `vi` to vitest imports in `field-renderer.test.tsx` (line 152 used `vi.spyOn`); (2) type the dataFetcher mocks in `page-contract.test.tsx` as `vi.fn<DataFetcher<Demo>>(...)` so `mock.calls[0][0]` resolves to the typed `DataFetcherParams<Demo>` instead of the empty-tuple `vi.fn` default.
- T11 follow-up (commit 2) — CI lint failed on second push: (1) ESLint `react-hooks/refs` rule flagged `field.value` / `field.onChange` / `field.onBlur` / `field.ref` access in `lib/scaffold/renderers/text.tsx` because RHF's `ControllerRenderProps` is a ref-bearing object; refactored to destructure once at component top before JSX. (2) `@typescript-eslint/no-explicit-any` flagged `field: any` in `form-page.tsx` Renderer type annotation; replaced with imported `FieldRendererProps` from `field-renderer.ts` (added narrow generic-variance cast at the `Controller render` call site to bridge `ControllerRenderProps<T, Path<T>>` ↔ `ControllerRenderProps<FieldValues, string>`).
