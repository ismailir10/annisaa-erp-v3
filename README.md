...
| 2026-04-24 | RLS enforces **tenant-scoped SELECT only**; INSERT/UPDATE/DELETE go through `service_role` (Prisma bypasses RLS). App-layer `tenantId` filtering in route handlers is the real write-side isolation — **a leaked service_role key bypasses RLS entirely.** `scripts/verify-rls-coverage.sh` is wired into CI to fail if any tenant-scoped model lacks an ENABLE + policy migration. Follow-up tracked: review CASCADE on `EmailLog.tenantId` + `OrgConfig.tenantId` before onboarding a second tenant (tenant deletion wipes audit trail). See [`docs/cycles/2026-04-24-stress-review-fixes-2.md`](docs/cycles/2026-04-24-stress-review-fixes-2.md). 18 tenantId/composite indexes dropped by `20260421000001_rls_security_cleanup` recreated by `20260424120000_recreate_rls_tenantid_indexes` (the other 19 were already restored by `20260421000002_rls_fk_indexes`). See [`docs/cycles/2026-04-24-stress-review-followups.md`](docs/cycles/2026-04-24-stress-review-followups.md) |
