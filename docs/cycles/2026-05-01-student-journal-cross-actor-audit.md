...
- **Post-PR CI fix #3 (commit `chore(rls):`):** Lint job's RLS coverage check flagged `InvoiceNumberSequence` (added in `20260426000001_invoice_number_sequence`) as missing both `ENABLE ROW LEVEL SECURITY` and a `CREATE POLICY` statement. Pre-existing red since #150 — not journal-related, but blocks CI. Added new migration `20260501000001_invoice_number_sequence_rls` enabling RLS + service-role bypass policy (matches finance internal-table pattern). Applied to staging via `prisma migrate deploy`. Local `bash scripts/verify-rls-coverage.sh` → "✓ RLS coverage OK: 24/24 tenant-scoped models".
- **Post-PR CI fix #4 (commit `chore(ci):`):** Lint job's `verify-api-auth.sh` flagged 27 routes as missing session helper. Pre-existing since #126: script's pattern allowlist did not include `requirePermission(` (introduced after script was written) and treated `Bearer ${SECRET}` cron auth + deploy health probes as missing. Fixed: (a) added `requirePermission(` to script's `AUTH_PATTERN`; (b) added `// @public` sentinel + comment explaining the alt auth on `app/api/health/xendit/route.ts` and `app/api/cron/finance-maintenance/route.ts`. Local: 128/128 routes ✓.
