...
>
> **Migrations landed (Phase 1 + Phase 2):** `00_extensions` (pg_trgm + pgcrypto), `01_tenancy` (Tenant/Campus/Program/AcademicYear/AcademicTerm), `02_identity` (User/Role/Permission/UserRole/RolePermission + composite FKs + RLS retroactive coverage on tenancy tables + Supabase Custom Access Token Hook), `03_employees` (Employee + EmployeeCampusAssignment + Campus(id, tenantId) backfill + Program.headEmployeeId FK wire-up), `04_classes` (ClassSection + Sentra catalog + TeachingDefault + SentraRotation + Program/AcademicYear/AcademicTerm composite-unique backfills + dayOfWeek CHECK on SentraRotation), `05_sessions` (ClassSession + SessionTeacher + SessionStatus + SessionTeacherRole enums + single-PRIMARY-per-session partial-unique guard + dayOfWeek CHECK on ClassSession), `06_audit_timeline` (AuditLog partitioned by month from day 1 — 18 monthly partitions inline 2026-05 → 2027-10 — with append-only `audit_log_block_update_delete()` trigger raising P0001 on UPDATE/DELETE for all roles incl. service-role + composite PK `(id, createdAt)` + AuditAction/TimelineVisibility enums + TimelineEvent w/ JSONB payload + GIN payload index), `09_regions` (Province/Regency/District/Village + RegencyType enum + trigram GIN on Village.name + public-read RLS — `idn-area-data` v4.0.1 vendored snapshot, ~91.6k rows applied via `prisma/seed/01-regions.sql`; non-tenant-scoped so it sits outside `verify-rls-coverage.sh`'s tenant set), `16_scaffold` (FileAsset + ExportJob + EmailLog + WebhookEvent + OrgConfig (singleton-per-tenant via column-level UNIQUE on tenantId) + Holiday + 4 enums (FileKind/FileStatus/ExportFormat/ExportJobStatus); soft-delete asymmetry: YES on FileAsset/OrgConfig/Holiday, NO on ExportJob/EmailLog/WebhookEvent — operational records), `07_students` (Household + Student + StudentIdentifier + StudentIdentifierSequence + composite FKs per spec §6.4 + tenant-scoped RLS + storage.objects RLS folded inline + soft-delete-aware partial-unique PRIMARY guard on StudentIdentifier (`WHERE "isPrimary" = true AND "deletedAt" IS NULL` — re-issues across cohorts retain history per spec §4.5). NIS allocator anchor — sequence row keyed on `(tenantId, academicYearId, programId)`. Advisory-lock is purely app-layer per `lib/students/nis-allocator.ts` via `pg_advisory_xact_lock(hashtext($tenantId || ':nis:' || $academicYearId))` inside `$transaction` (no SQL helper function in the migration). Soft-delete: YES on Household/Student/StudentIdentifier, NO on StudentIdentifierSequence — operational/append-only), `08_guardians` (Guardian + StudentGuardian + GuardianInvitation + composite FKs per spec §6.4 + tenant-scoped RLS + relationship-scoped partial-unique PRIMARY guard on StudentGuardian (`WHERE "isPrimary" = true AND "deletedAt" IS NULL` scoped to `(studentId, tenantId, relationship)` — diverges from migration 07's StudentIdentifier global-PRIMARY shape so PRIMARY FATHER + PRIMARY MOTHER coexist for two-parent families per Indonesian PAUD admission). Composite FK on Guardian.userId uses Postgres-15.4+ column-list `ON DELETE SET NULL ("userId")` syntax — preserves §6.4 tenant alignment when User is hard-deleted. Schema declares Guardian.user as a SINGLE-column Prisma relation (NOT composite) to dodge Prisma issue #25061 (composite SetNull would null tenantId at runtime); `prisma migrate dev` will detect drift — REJECT regeneration in PR review. GuardianInvitation.token is a 32-byte base64url app-generated string with global UNIQUE (no partial WHERE). Soft-delete: YES on Guardian/StudentGuardian, NO on GuardianInvitation — operational record, status enum carries lifecycle. storage.objects RLS NOT re-added (already on staging from migration 07)). **Seeds landed:** `00-tenant`, `01-regions.sql`, `02-campuses`, `03-programs`, `04-academic-year`, `05-system-roles`, `06-permissions`, `07-sentra` (8 PAUD catalog rows). **Audit redactor:** `scripts/generate-audit-redactor.ts` reads `/// @PII (redact|mask:last4)` schema annotations and emits `lib/audit/redactor.ts` deterministically (committed, idempotent). Currently annotates `Employee.nik` (redact) + `Employee.phone` (mask:last4) + `Student.nik` (redact) + `Guardian.nik` (redact) + `Guardian.phone` (mask:last4); CI gate `scripts/verify-pii-annotations.sh` asserts all five (5/5 as of `p2-guardians`).
>
> **RLS coverage guard:** `scripts/verify-rls-coverage.sh` runs in **strict** mode as of `p1-identity-rls` (2026-05-05) — every tenant-scoped Prisma model must have both `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` and at least one `CREATE POLICY` in `prisma/migrations/**`. Current count: **32 / 32** as of `p2-guardians` (3 new tables added 29 → 32 — Guardian/StudentGuardian/GuardianInvitation). The mid-rebuild parser-regression floor is set to 5 (will pass-through naturally as remaining Phase 2 cycles push the count past 35).
...
| `scaffold.md` | Per-entity registry conventions (schema/entity/policy shape, dataFetcher contract, scope mapping, FileKind allowlist Partial, split-view FK precedent, partial-unique drift trap) | `lib/entities/**`, `.claude/standards/scaffold.md` |
| `auth.md` | Google OAuth callback, `getSession()` contract, JWT custom-claim hook, demo-mode HMAC cookie, `?next=` validation, multi-tenant invariant enforcement | `app/api/**`, `app/auth/**`, `lib/auth/**`, `proxy.ts` |
