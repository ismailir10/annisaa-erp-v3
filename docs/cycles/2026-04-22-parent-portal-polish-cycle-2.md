...
- [x] **T14 — Dashboard IA: drop tagihan list, add RecentActivity feed. Dep: T13 (ordering on page). Subagent-heavy.**
...
- [x] **T10 — Standards + README doc-sync + PageHeader primitive + parent route sweep. Dep: T1–T9, T11–T15.**
...
- T10: subagent dispatch (general-purpose). New `components/portal/page-header.tsx` primitive + `components/portal/__tests__/page-header.test.tsx` (4 tests). Five parent routes swept to adopt `<PageHeader>`: `app/parent/page.tsx`, `app/parent/invoices/client.tsx`, `app/parent/attendance/client.tsx`, `app/parent/student-journal/page.tsx` (dropped leading `<BookHeart>` icon from title — import retained for empty-state block), `app/parent/reports/page.tsx`. `.claude/standards/portal.md` gained 5 new sections (Component Reusability Layers with layer table + 2nd-instance rule, PortalHeader, PortalBottomNav, PageHeader, Spacing Scale table) + portal primitive inventory at EOF + note that teacher text-size sweep is complete. No further README change needed (recent-cycles bullet staged in T12 already accurate).
- T14: subagent dispatch (general-purpose). New `lib/validations/parent-activity.ts` + `lib/parent-activity.ts` (shared helper consumed by both page + API route — cleaner than HTTP self-fetch). New `app/api/parent/children/[id]/activity/route.ts` + `components/parent/recent-activity.tsx` with icon-per-kind + relative time. `lib/format.ts` gained `formatRelativeTime` (Indonesian — "baru saja" / "X menit lalu" / "X jam lalu" / "X hari lalu" → `formatDateShort` after 30 days). 6 event kinds wired: ATTENDANCE_MARKED (StudentAttendance, `isVoided=false`), NOTE_POSTED (StudentJournalNote + batched User name lookup — no relation in schema), JOURNAL_ENTRY (StudentJournalEntry, deduped to first per date), INVOICE_ISSUED (Invoice, `sentAt ?? createdAt`, excludes DRAFT), PAYMENT_RECEIVED (Invoice, `paidAt`), REPORT_PUBLISHED (StudentAssessment + template.name, `publishedAt ?? createdAt`). `app/parent/page.tsx` server-fetches activity via shared helper, renders `<RecentActivity>` below QuickLinkCard grid. `app/parent/unpaid-invoices-table.tsx` action column normalised: Bayar (`size=icon, variant=default`) + Lihat (`size=icon, variant=ghost`), both 32 px with `aria-label`. Text "Bayar"/"Lihat" now sr-only via aria.
...
- End-of-cycle gate: `npm run build` green; `npx vitest run` → 240 passed / 42 todo / 2 skipped (282 total). Banned-size grep across `app/parent app/teacher components/parent components/teacher components/portal` returns zero.
- Initial Playwright run was DB-blocked locally (Supabase pooler hostname rotated; `.env` carried stale `postgres.jzhujpqaxyeeokgexerc`). After `.env` refresh + `prisma generate`, real bug surfaced: parent dashboard server component crashed with *"Functions cannot be passed directly to Client Components"* — `app/parent/page.tsx` (RSC) was passing Lucide icon function refs into `<QuickLinkCard>` which was marked `"use client"`. Fix: drop `"use client"` from `components/parent/quick-link-card.tsx` (component has no state/effects/handlers — pure render, RSC-safe). Two test selectors also went stale this cycle: `[title='Keluar']` → `[aria-label='Keluar']` (PortalHeader uses aria-label only) and the dashboard "unpaid invoices section visible" test → "recent activity section visible" (T14 dropped the embedded table). Final Playwright: `npx playwright test` → **36 passed / 2 skipped** in 35.8s.
