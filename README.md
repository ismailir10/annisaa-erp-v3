...
- **Teacher** — check-in/out, attendance calendar, Nilai Siswa (with live-announced pre-publish missing-score warning), Buku Penghubung (school scope), tappable salary slip rows → in-app detail (`/teacher/slips/[id]`) + PDF download per row, profile.
