...
- **Task 5 follow-up (review-flagged)** — Email subject in `lib/email/send-slip.ts` updated `"Slip Gaji ${period} — An Nisaa' Sekolahku"` → `"Slip Gaji ${period} — Talib"` so subject matches sender display + body brand. Four runtime `NEXT_PUBLIC_APP_URL` fallbacks updated from `https://annisaa-erp-v3.vercel.app` to `https://talib.annisaasekolahku.com`: `lib/email/send-slip.ts`, `app/api/slips/[payrollItemId]/pdf/route.ts`, `app/api/guardian/invoices/[id]/pdf/route.ts`, `app/api/payroll/[id]/send-slips/route.ts`. Test fixtures in `lib/__tests__/xendit-*.test.ts` and `app/api/__tests__/auth-callback-origin.test.ts` intentionally left unchanged (they assert URL-parsing behavior on fixed inputs, not brand strings).
