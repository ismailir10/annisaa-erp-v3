...
- Task 5: People seed — `scripts/reseed/rng.ts` (seeded mulberry32 PRNG — reproducible runs), `scripts/reseed/names.ts` (Indonesian/Islamic name pools, occupation + income + education enums), `scripts/reseed/people.ts` exports `SEED_COUNTS`, pure planners `planStudents` (200 total: 180 ACTIVE + 20 GRADUATED with preserved Bilal Hakim + Ahmad Faris Abdullah at fixed indexes, DCARE→Taman-Aster-only respected, one-tier historical rollback), `planParents` (one mother per student + 2 preserved guardians), `planEmployees` (25 teachers inc. IR01/WR03 + 3 support staff S01-S03), and DB writer `seedPeople(prisma, org, preservedUuids)` returning id lookup maps. 14 vitest cases covering counts, preserved fixtures, campus-program gate, y24/y25 section-key coverage, reproducibility. Reviewer flagged silent-enrollment-skip risk — replaced `if (sectionId)` guards with throws + added 2 key-coverage tests.
- Task 8: Invoice + Xendit seed — `scripts/reseed/invoices.ts` exports `buildInvoicePeriods` (10 periods Jul-2025→Apr-2026, last 3 = LIVE_XENDIT), `computeInvoiceTotal` (per-program SPP+Makan+Kegiatan), and `seedInvoices(prisma, org, people, studentPlan, parentPlan, opts)` writer that creates Invoice + 3 InvoiceLine rows per ACTIVE student per period (program-specific amounts via FEE_SCHEDULE), Payment row for HISTORICAL_PAID months, plus 10 PAID invoices per GRADUATED student covering 2024/25. Live Xendit calls use `referenceId = staging-tagihan-{invoiceId}`, throttled at concurrency 5 with 200ms tail-pace + exponential backoff on 429 (3 retries). Idempotent: skip if `invoice.xenditSessionId` already set. `xenditCaller` injectable for tests; defaults to `createXenditSession` from `lib/xendit/client.ts`. 4 vitest cases cover period split + per-program totals.
- Task 7: Payroll seed — `scripts/reseed/payroll.ts` exports `buildPayrollPeriods` (22 monthly periods 2024-07→2026-04, current month DRAFT, others APPROVED) and `seedPayroll(prisma, org, people, employeePlan)` writer that creates `EmployeeSalaryValue` rows for every employee × every salary component (base salary varies by jabatan + tenure: teacher 4.8M+, Admin TU 4.5M+, Kasir 4.0M+, OB 3.2M+, +200k/yr tenure bonus), 22 `PayrollRun` rows attributed to the SUPER_ADMIN preserved user, and `PayrollItem` + `PayrollItemLine` for every employee active by the period start (4 lines: gaji_pokok, transport, kehadiran, BPJS deduction). 3 vitest cases covering period count, DRAFT status assignment, and last-day-of-month math.
- Task 6: Operational history — `scripts/reseed/operations.ts` exports pure helpers `enumerateSchoolDays` (Mon-Fri minus holidays from `prisma/data/holidays.ts`) and `applyDensityRule` (last-N-days full + Mon/Wed/Fri sample older), plus DB writer `seedOperations(prisma, org, people, studentPlan, employeePlan)` that creates `StudentAttendance` (active 2025/26 dense+sampled, graduated cohort 2024/25 sampled), `AttendanceRecord` for every employee from hireDate→today (PRESENT 80% / LATE 12% / ABSENT 5% / LEAVE 3%), and `StudentJournalEntry` (last 14 school days × ACTIVE × 3 SCHOOL-scope indicators, `checked` 70%). Updated `SeedOrgResult` with `journalIndicatorIdsByScope` so journal seed uses only SCHOOL-scope indicators (matches `@@unique scope` constraint). 6 vitest cases for the pure helpers (weekend exclusion, holiday exclusion, density window edge cases).
...
- Task 5: `npx vitest run scripts/reseed` — 71/71 passing (+13 people cases incl. y24/y25 key coverage). `npm run build` — clean. Reviewer flagged silent-enrollment-skip; replaced with throw.
- Task 6: `npx vitest run scripts/reseed` — 77/77 passing (+6 operations cases). `npm run build` — clean.
- Task 7: `npx vitest run scripts/reseed` — 80/80 passing (+3 payroll period cases). `npm run build` — clean.
- Task 8: `npx vitest run scripts/reseed` — 84/84 passing (+4 invoice cases). `npm run build` — clean.
- Task 9 + review fixes: `npx vitest run scripts/reseed` — 87/87 passing (+3 DATABASE_URL guard cases). `npm run build` — clean. Both reviewers (feature-dev + superpowers) agreed on three findings — all fixed.
- End-of-cycle gate: `npm run build` clean, `npx vitest run` 457 passed / 42 todo / 2 skipped across 62 files, `npx playwright test` 43 passed / 2 skipped (admin + teacher + parent + design-system specs all green). Infrastructure cycle did not change frontend behavior; e2e runs unchanged.
- Task 9 review fixes: end-of-cycle reviewer pass surfaced 3 high-confidence issues. (a) MED-security split-brain: `DATABASE_URL` host wasn't cross-checked against `STAGING_SUPABASE_REF`, so a typo could TRUNCATE one project while wiping auth in another — added URL parse + ref-presence check covering both direct (`db.<ref>.supabase.co`) and pooler (`postgres.<ref>@aws-…`) formats with 3 new tests. (b) `operations.ts` defaulted `today` to the hard-coded cycle date `2026-04-25`, creating a stale ceiling on every future reseed — now defaults to `new Date()`. (c) README didn't warn operators that re-running after a mid-stage crash fails on duplicate Tenant creation — added explicit partial-failure clause.
- Task 9: orchestration + docs — `scripts/reseed-staging.ts` wires guards → countdown → ensurePreservedAuthUsers → wipeApplicationData → seedOrg → seedPeople → seedOperations → seedPayroll → seedInvoices, with progress logging at each step and a final summary. `SeedPeopleResult` extended with `studentPlan/employeePlan/parentPlan` so downstream seeders consume them without re-planning. README "Reseeding staging" section added under `## Environments` with env-var checklist, snapshot-first reminder, command, preserved-account list, and rollback path. Final gate: `npx vitest run` — 84/84 passing across 8 files; `npm run build` — clean. End-to-end live execution against the staging Supabase project is left to the operator (requires service role key + sandbox Xendit key not present in this worktree).

## Ship Notes

**Migrations:** none. This cycle adds operator tooling only — no schema changes.

**New env vars (operator-side, not committed):** `STAGING_CONFIRM`, `STAGING_SUPABASE_REF`, `XENDIT_SECRET_KEY` (sandbox), plus the existing `NEXT_PUBLIC_SUPABASE_URL`, `DATABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`. The script refuses to run unless every guard passes.

**Manual smoke-test sequence (operator runs after merge):**

1. Take a manual Supabase snapshot of the staging project via dashboard → Database → Backups → "Create backup".
2. Set the six env vars above in the operator's local `.env.local` (or shell export). Verify `STAGING_SUPABASE_REF` matches the staging project (NOT prod).
3. From the worktree or main checkout: `npm run reseed:staging`. Expect ~5 minute runtime (most of it is the 3 months × 180 students = 540 live Xendit sandbox calls at ~5 rps).
4. Verify in the Supabase SQL editor: `SELECT count(*) FROM "User"` → 6, `SELECT count(*) FROM "Student"` → 200, `SELECT count(*) FROM "Invoice" WHERE "xenditSessionId" IS NOT NULL` → ~540.
5. Log in as `ismailir10@gmail.com` on staging — confirm SUPER_ADMIN sidebar, current 2025/26 enrollment list, payroll history (Jul-2024 → Apr-2026), and tagihan list with clickable Xendit hosted-checkout URLs for Feb/Mar/Apr 2026.
6. Log in as `rightjet.hq@gmail.com` (Ibu Nurul) — confirm Bilal Hakim appears with attendance history + journal entries + invoice list.

**Rollback plan:** restore the manual Supabase snapshot taken in step 1 via dashboard → Database → Backups → "Restore". The script is idempotent on partial failure — re-running it from a consistent wiped state is safe; restore is only needed if the operator wants to abandon the reseed entirely.
