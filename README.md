...
- **Admin** — dashboard (shadcn `ChartContainer` + `components/admin/dashboard/*` split; Pending Admissions row + Recent Activity feed via `AuditLog`; `Promise.allSettled` per-section degradation), employees, attendance (daily/monthly/LEAVE override), payroll (`DRAFT → APPROVED → EXPORTED → SLIPS_SENT`), penilaian monitor (read-only walas-weekly + sentra-daily `AssessmentEntry` completion via `GET /api/admin/penilaian`, gated by `assessments.read`), settings.
