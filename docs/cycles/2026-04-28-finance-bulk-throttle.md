...
- T5 — Synthetic timing test — new `lib/finance/__tests__/xendit-retry.timing.test.ts` exercises `limit(2)` + `withXenditRetry` directly with `vi.useFakeTimers()`. Two cases: realistic mix (23 happy + 1 × 429-retry + 1 × 5xx-retry) <30s simulated, 429-storm worst case (25 × 429-then-200) <59s simulated (60s ceiling regression guard). Test docblock explicitly notes that the spec's earlier 5+15+5 mix was arithmetically inconsistent with the 30s budget at concurrency=2 and aligns Case A with the spec's other "≤5% retries → ~21s/chunk" claim.
- T7 — README update — folded into T4 commit (the new `/api/health/xendit` route triggered the doc-sync hook's narrow rule for `feat:` commits touching `app/**` or `lib/**`). Finance module narrative now mentions concurrency=2 + 1s pacing + 429 retry trim + the deploy-time probe with cycle-doc cross-reference.
...
- T6.5 — Pre-ship reviewer caught missing `export const maxDuration = 60` on `app/api/invoices/generate/batch/route.ts` (spec budget-math section explicitly committed to it; the 60s ceiling is what the entire 429-storm fitting argument rests on). Added the export with an explanatory docblock above the POST handler. Gates re-run clean: `npm run build` clean, `npx vitest run --no-file-parallelism` 807 passed / 42 todo / 0 fail.
