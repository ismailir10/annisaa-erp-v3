...
1. **Bilal Hakim has no `StudentEnrollment`** — he has 5 attendance records (which require a `classSectionId`) but no enrollment row. So `getParentWithChildren` returns `className: null` for his child tab, and the attendance query has an implicit classSectionId that we need to discover.
...
- Populate `Invoice.parentId` for all invoices where the student has a primary guardian with a known `parentId`:
  ...
  - Bilal Hakim invoice → `cmnriprp7000105jxvzydico7`
...
Seed rows for **both** `st_4` and Bilal Hakim's studentId. Use their known `classSectionId`. Vary statuses realistically:

| Date | Ahmad Faris | Bilal Hakim |
|---|---|---|
...
### [x] Task 5 — Create PUBLISHED StudentAssessment + scores for Bilal Hakim
