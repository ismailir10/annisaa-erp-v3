/**
 * Inline retry helper for payment-gateway calls. Gateway-agnostic (moved
 * from `lib/xendit/with-retry.ts` in cycle 2026-07-27-doku-payment-gateway
 * T1 — the retry loop only depends on `GatewayApiError`, never on Xendit
 * specifics). `lib/xendit/with-retry.ts` re-exports this as `withXenditRetry`
 * so existing consumers/tests are unaffected.
 *
 * Wraps any function returning a Promise. Catches `GatewayApiError` and
 * retries on `retriable === true` up to `MAX_ATTEMPTS` total. Honors
 * `error.retryAfterMs` (already capped at 3000ms by the adapter) when
 * present, otherwise uses `BACKOFFS_MS` by attempt index.
 *
 ...
 * Log prefix kept as `[XENDIT ATTEMPT]` verbatim — this is a gateway-agnostic
 * helper today, but the prefix is an established ops-grep contract and
 * renaming it is out of scope for zero-behaviour-change T1.
 *
 * `result` convention:
 * - `success`  — the attempt's `fn()` resolved.
 * - `transient` — caught a `GatewayApiError` with `retriable === true`. This is
 ...
 * - `hard` — caught a `GatewayApiError` with `retriable === false`, OR caught a
 *   non-`GatewayApiError` (programmer bug, etc). Re-thrown immediately.
 *
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (!(err instanceof GatewayApiError)) {
        ...
        // hard fail so we don't loop on a non-gateway error.
