...
- **Fees** (11c): seeded 3 `FeeComponentDef` (`spp`, `daftar_ulang`, `seragam`) + 12 `ProgramFeeStructure` rows (4 programs × 3 components). PFS written via `$executeRawUnsafe` because DB has `tenantId NOT NULL` + `status` columns not declared in `schema.prisma` (pre-existing schema drift — separate task spawned).
