...
- **T14** — hardcoded Tailwind palette → status tokens. `classes/{client,[id]/client}.tsx`: removed both local `HEALTH_TONE` maps, consume `healthTone()` from T1; archived-year banner blue → `border-status-leave bg-status-leave-subtle text-status-leave-text`. `admissions/page.tsx` sibling-detect banner amber → status-late. `KategoriIndikatorBuilder.tsx` lockNotice amber → status-late. `portal/week-grid.tsx` admin-edit amber → status-late-subtle (+ `Pencil size={10}→{12}`). `enrollments/[id]/page.tsx` success `text-emerald-700` → `text-status-present-text`. Acceptance grep across `app/admin`+`app/teacher`+`app/parent`+`components/{admin,teacher,parent,portal,student-journal,attendance}` = 0 (excluding PDFs).
- **T9** — hand-rolled `style={{ width: % }}` progress bars → Shadcn `<Progress>`: `components/teacher/leave-sheet.tsx` (annual + sick, sick keeps `bg-status-leave` via indicator override), `app/admin/student-journal/{monitoring,classes/[id]}/page.tsx` CompletionBar. Grep `style={{ width: \`${…}%\`` in scope = 0. (Shared `CompletionBar` extraction deferred — inline `<Progress>` used instead.)
- **T11** (partial) — plain "Memuat…" loading → row-shaped `<Skeleton>`: `semesters/[id]/{themes,objectives}/client.tsx` (4 instances), `teacher/assessments/center/[center]/client.tsx` full-roster `<Loader2>` → Skeleton list. New `loading.tsx` files + single-Skeleton early-return removals deferred.
- **T13** (partial — error handling): `teacher/attendance/page.tsx` prefetch `console.log` → `toast.error`; `(hr)/employees/page.tsx` 3× silent `console.error` `.catch` (campuses/positions/stats) → `toast.error`. Remaining T13 files (payroll stats/compare, employee-attendance, leave-requests, salary-components, slips, semesters fetchJson branching) deferred.
...
- **Cycle gate (cumulative, all landed tasks)** — `npm run build` green; `npx vitest run` green (220 files / 2127 tests / 42 todo); `npm run lint` 0 errors (pre-existing warnings only). Playwright deferred to the required CI `Playwright E2E` check per #368 (this harness cannot run Playwright locally). New e2e specs (skala-consistency, journal-author, optimistic-save-revert, daftar-brand) from T24 **not added this cycle** — deferred; the existing 33 specs remain the regression net.
- **Tasks landed this cycle:** T0 (cycle doc), T1 (color/status tokens + PDF brand-tokens + enrollments extraction), T2 (level-presentation single source), T3 (/daftar re-skin), T4 (login onDark), T5-partial (destructive: withdraw/raport/leave), T9 (Progress bars), T11-partial (Skeleton loading), T13-partial (error toasts), T14 (palette→tokens), T15 (celebration-gold className), T16 (banned sizes), T22-partial (voice copy), T23-partial (dead-code deletes).
- **Tasks deferred to follow-up cycle(s)** (feature-scale — rushing them risks regressions; called out for the reviewer): **T6** (teacher sessions per-tap optimistic save), **T7** (center assessments per-tap save), **T8** (student-journal per-tap + WeekGrid editable + admin placebo), **T10** (DataTable migration: penilaian/raport/classes), **T12** (EmptyState sweep), **T17** (HR chrome cleanup), **T18** (academic Recipe 2 + filter bar), **T19** (journal audit trail + author visibility — *real-harm: anonymous notes*), **T20** (curriculum drill-down UX), **T21** (raport PDF formality + editor sticky bar). Plus the partial-task tails noted inline above.

## Ship Notes

Filled by `/ship`.

- **DB migrations:** none. All changes are presentational / token / copy. No schema touched (`student-journal` author joins would have used existing `User.name` — but T19 is deferred).
- **Env vars:** none.
- **Rollback:** `git revert` per commit — commits are atomic per task/cluster (12 commits on `feat/ui-sweep`). No shared mutable state between commits.
- **PDF brand-token centralization** (`lib/pdf/brand-tokens.ts`): `@react-pdf/renderer` is server-side with no DOM/CSS-var access, so hex is unavoidable; centralization is the mitigation. Reviewer Q anticipated.
- **Label-policy deviation (T2):** preserved `EMERGING` short label "Belum" instead of the doc's literal "Belum Mampu" to avoid contradicting raport's long-form "Mampu Belum Konsisten" for the same level. User confirmed "fully unify" policy (single module, short+long variants); the short-string choice is flagged for reviewer sign-off.
- **Deferred work is feature-scale, not cosmetic** — T6/T7/T8 (per-tap optimistic save), T19 (journal audit trail + author visibility), T21 (raport PDF formality), T10 (DataTable migration), T17/T18/T20 (module UX), T12 (EmptyState). Recommend a follow-up cycle (or per-task PRs) rather than expanding this PR. The two "real user harm" items (T8 lost journal input, T19 anonymous notes) should be prioritized next.
- **Merge:** product-builder (opencode/glm-5.2) does NOT self-merge. PR labeled `needs-cto-review`. CTO (Claude) to preview-verify on the Vercel deployment (Chrome MCP, role-scoped accounts per `.claude/verify-accounts.json`) across the landed surfaces — `/daftar`, login, admin students withdraw, raport editor, leave-requests, classes health badges, teacher assessments skala chips, parent reports — then merge once the four required checks are green.
