...
**Do not rely on the notification alone.** An audit on 2026-07-29 found that *no* DOKU notification has ever verified — every DOKU payment to date was credited by an admin pressing "Perbarui pembayaran", while the Xendit webhook has real deliveries. An hourly cron, `POST /api/cron/reconcile-payments`, therefore polls the gateway for every outstanding invoice that has a payment link and credits anything that settled, routing through the same processor as the webhook (so it is idempotent and cannot double-credit). It is the safety net, not a replacement: a delivered notification still credits immediately and the sweep then no-ops.
