...
| 2026-04-26 | Finance bug fixes round 2: `createXenditSessionForInvoice` now flips status:SENT atomically inside the per-invoice advisory-lock tx (closes TOCTOU between helper and void/webhook); `updateInvoiceSchema` narrowed to `["DRAFT","SENT"]` (every other transition has a dedicated route); `revalidateTag("parent-invoice-list")` parity wired into all 8 status-change paths (webhook, payments, void, manual create, batch generate, bulk send, retry, PUT); manual-payment POST enforces `recordPaymentSchema` via `safeParse` (drops ad-hoc `Number(body.amount)` + `?? "CASH"` fallback); webhook `payment_session.completed` refuses to credit when `amount` missing (was silently falling back to `invoice.totalDue`, masking partial captures). | Closes residual server-side correctness gaps surfaced by post-PR-141 finance UX re-audit. Parent-list cache stale was the most-visible — paid invoice still showed "Belum Bayar" until TTL. See [`docs/cycles/2026-04-26-finance-bug-fixes-round-2.md`](docs/cycles/2026-04-26-finance-bug-fixes-round-2.md) |
