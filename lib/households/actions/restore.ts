...
// restoreHousehold — Clears `deletedAt` + emits RESTORE audit row.
//
...
export async function restoreHousehold(id: string): Promise<ActionResult<Household>> {
  const session = await getSession();
  if (!session) return { ok: false, error: "UNAUTHENTICATED" };

  try {
    assertScope(session, householdPolicy, "restore");
  } catch {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    if (householdPolicy.auditActions.includes(AuditAction.RESTORE)) {
      await writeAuditLog(
        {
