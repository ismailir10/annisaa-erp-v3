...
⁵ **`DOKU_CHECKOUT_VERSION` — an open experiment, not a tuning knob.** DOKU support stated (2026-07-30) that `additional_info.override_notification_url` "is supported for the API V2 Checkout". Their docs publish only `POST /checkout/v1/payment`, but `/checkout/v2/payment` is genuinely routed on both hosts — an unauthenticated probe returns `404 No static resource` for `/checkout/v9/payment` and DOKU's real header validation for v2. If notification dispatch is wired on v2 and not v1, that explains why this merchant has never received one. v2's request-body contract is undocumented and untested, so the flag defaults to `v1` and getting it wrong cannot take payment links down. Run `node scripts/doku-probe-checkout.mjs --version v2` against the sandbox first (credentials from the environment, never printed; `--notify <bin-url>` to see whether DOKU sent anything at all, since our own webhook rejects a bad signature before writing a `WebhookEvent` row). Flip the flag only after a notification is observed arriving. Full procedure: [`docs/cycles/2026-07-30-doku-checkout-v2-notification.md`](docs/cycles/2026-07-30-doku-checkout-v2-notification.md).
