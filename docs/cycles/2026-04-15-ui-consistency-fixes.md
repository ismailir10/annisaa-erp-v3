...
2. [ ] **Standardize action column pattern** — Ensure all DataTable action columns use `DataTableRowActions` component with standardized "Lihat" button + dropdown (⋮) for Edit/Deactivate. Apply to admin portal list pages first.
...
**Superseded by `2026-04-16-ui-audit-phase2.md`.** Tasks 2–10 were never built. The new cycle re-audits the entire codebase with broader scope (admin + teacher + parent portals, API security, test health) and incorporates the remaining items from this cycle along with new findings. No code was shipped from this cycle — only Task 1 (audit verification) was completed.
