...
if [ ! -f "$ROLE_FILE" ]; then
  echo "[check-role] Session role not set. Assistant: ask the user to pick 'cto' or 'product-builder' using AskUserQuestion (include your own model name in the question), then write $ROLE_FILE with two lines: role=<cto|product-builder> and model=<your-model-id>. Do not run /spec, /build, or /ship until this is done."
  exit 0
fi
...
echo "[check-role] Last session: role=$ROLE, model=$MODEL. IMPORTANT: If the user's opening message declares a role ('you are cto', 'act as product-builder', 'i am cto', etc.), write $ROLE_FILE immediately (role=<declared> and model=<your-model-id>) before any other action, even though the file already exists. Do not silently inherit the previous session's role."
