...
| Date | Decision | Why |
|---|---|---|
| 2026-05-14 | Curriculum C4 (in flight) — `AssessmentEntry` + `AssessmentSource`/`LearningCenter` enums; walas weekly mobile UI at `/teacher/assessments/weekly` writes via `POST /api/teacher/assessment-entries`; assessments hub + walas home quick card route walas to it | First write path against curriculum indicators; English-only code identifiers (Indonesian only in UI copy) per CTO directive — see [cycle](docs/cycles/2026-05-14-curriculum-c4-weekly-assessments.md) |
