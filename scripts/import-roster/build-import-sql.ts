...
async function main() {
  ...
  p(`END $$;`)
  p(``)
  p(`-- 1. Assert every Kelas resolves to EXACTLY ONE ClassSection for the AY (fail early).`)
  p(`--    The sheet has no campus column, so a Kelas is matched by name alone. Class names`)
  p(`--    used to be globally unique within a year, which made that safe; since cycle`)
  p(`--    2026-07-29-class-picker-year-scoping the ClassSection unique key is`)
  p(`--    (tenantId, academicYearId, campusId, name), so two campuses MAY legitimately hold`)
  p(`--    the same name in one year. If that ever happens for a Kelas in this sheet the`)
  p(`--    enroll join at step 5 fans out to two rows per student and dies on the deterministic`)
  p(`--    'ime_'||tmpid primary key — a confusing failure halfway through the transaction.`)
  p(`--    Assert it up front instead, with a message that names the ambiguous Kelas and tells`)
  p(`--    the operator the sheet needs a campus column before this import can run.`)
  p(`DO $$ DECLARE bad text; ambiguous text; BEGIN`)
  ...
  p(`  SELECT string_agg(DISTINCT s.kelas, ', ') INTO ambiguous FROM public._roster_stage s`)
  p(`  WHERE (SELECT count(*) FROM public."ClassSection" cs WHERE cs.name=s.kelas AND cs."academicYearId"='${AY}') > 1;`)
  p(`  IF ambiguous IS NOT NULL THEN RAISE EXCEPTION`)
  p(`    'ambiguous Kelas (matches >1 ClassSection across campuses for the AY): % — the sheet needs a campus column to disambiguate', ambiguous;`)
  p(`  END IF;`)
  p(`END $$;`)
  p(``)
