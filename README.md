...
| **students** | Student lifecycle — auth-proxied photo upload, guardians (one parent record shared across siblings; Tambah Wali searches first, with an overridable duplicate warning on name, phone, NIK or email), enrollments (one school plus one day-care placement per year; age bands advisory, overridden with an audited reason), admissions (`/daftar` inquiry → token → form → convert), CSV export. Detail is a single-scroll dossier — anchor nav, collapsible sections, rail with an outstanding-balance tile, NIK/KK masked, read-only Keuangan / Keringanan / Jurnal that deep-link out |
