...
Goal: tighten the 6-state v1 CRM into a richer 8-state machine with per-transition audit + cleaner ACCEPTED side-effect bundle.

Cycles:
- **2.1 — admission-state-machine-library** (~6 files). Lift `lib/admission/state-machine.ts` from v2 verbatim (pure algebra, no DB). Map v1's `INQUIRY|VISIT_SCHEDULED|VISITED|ADMITTED|REGISTERED|CANCELLED` to v2's 8-state vocabulary (`DRAFT|SUBMITTED|UNDER_REVIEW|INTERVIEW_SCHEDULED|OFFER_EXTENDED|ACCEPTED|REJECTED|WITHDRAWN`). Migration adds the new states (additive enum extend). Existing rows backfill via SQL UPDATE per status mapping table. Vitest covers every legal/illegal transition.
- **2.2 — admission-transitions-audit-bundle** (~14 files). 5 transition server actions (`submit`, `review`, `accept`, `reject`, `withdraw`) — each wraps `assertTransition` + writes `AuditLog` row + sends email (review-pending / accepted / rejected templates). Refactor `convert-to-student` into the `accept` transition's side-effect (auto Household + Student + Guardian creation with NIK-dedup). Admin review detail page gets state-aware action buttons. **Permission scope (per user §7 q3):** admin + principal only; no `admission_officer` role exists in v1, defer if needed later.

Phase 2 verdict gate: Playwright spec walks SUBMITTED → UNDER_REVIEW → OFFER_EXTENDED → ACCEPTED end-to-end; verifies side-effect bundle creates exactly one Household + Student + N Guardians atomically.
