...
- Task 5: `e2e/parent.spec.ts` — added "home and /parent/invoices agree on outstanding (UAT-2026-05-03 INV-01)" cross-page Playwright spec. Captures home count (or detects "Lunas semua"), navigates to `/parent/invoices`, asserts: (count===0 → list shows "Lunas semua"); (count>0 → list shows "Belum dibayar" header OR "Lunas untuk {name}" sibling-switcher branch); naive `/^Lunas semua$/` standalone count must be 0 when household has outstanding. Reviewer caught a flakiness vector (probing `.isVisible()` before paint complete → vacuous green); fixed by adding explicit `await expect(...).toBeVisible({ timeout: 10_000 })` gate before count scrape. All 8 parent.spec tests pass against `DEMO_MODE=true npm run start`.
