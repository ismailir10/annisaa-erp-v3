...
- [ ] **T4 — `scripts/cleanup-demo-orphans.ts` one-off cleanup.** New file. Imports prisma client; runs `prisma.$transaction` with the same ordering as `app/api/demo/admission/[id]/effects/route.ts` DELETE handler:
...
- [ ] **T5 — Playwright `e2e/admin/students.spec.ts` regression fix + extension.** Edit existing spec:
