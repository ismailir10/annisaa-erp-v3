...
| Date | Decision | Why |
|---|---|---|
| 2026-05-18 | Kesiswaan CRUD parity (in flight): `lib/constants/parent-options.ts` superset for education/occupation/income; entity-generic `lib/storage` adapter + auth-proxied `Student.photoUrl` upload (≤2 MB JPG/PNG, magic-byte validated, never under `public/`); Student-metadata key/value editor with explicit-null clear on `PUT /api/students/[id]` | KTP/KK (T14) needs the adapter; metadata grid was read-only; PUT couldn't clear `metadata` — see [cycle](docs/cycles/2026-05-18-kesiswaan-crud-audit.md) |
