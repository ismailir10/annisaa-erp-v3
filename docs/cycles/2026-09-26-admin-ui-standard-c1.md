...
- [x] **T1 — DataTable mobile contract + AdminTabs + toolbar reset + breadcrumb labels + Dossier mobile dedupe.** `components/ui/data-table.tsx` (sticky action column, `meta.priority`, edge fade), `components/admin/admin-tabs.tsx` (owned scrollable list), `components/ui/data-table-toolbar.tsx` (reset only when active), `config/admin-nav.ts` (segment labels), Dossier nav/rail. *Accept:* Vitest for priority/sticky classes and breadcrumb labels; 390px screenshots show actions on employees/leave-requests.
- [x] **T2 — Primitives: DatePicker, RupiahInput, AsyncCombobox**; rebase three pickers; delete `form-field.tsx`, `combobox.tsx`. *Accept:* unit tests per primitive; picker tests still green.
...
- Task 1: shared layer — `components/ui/data-table.tsx` (ColumnMeta `priority: "low"` → `hidden md:table-cell`; actions column / `meta.sticky: "right"` sticky-right below md with opaque bg + left shadow; right-edge fade only on tables without a sticky column), `components/admin/admin-tabs.tsx` (`AdminTabsList`/`AdminTabsTrigger` own a single-row horizontally scrolling strip; `AdminLinkTabs` matches), `app/admin/fees/page.tsx` (removed the wrapping override), `components/ui/data-table-toolbar.tsx` ("Atur Ulang" rendered only when something is active, ghost + X icon), `config/admin-nav.ts` (segment labels Kelas/Siswa/Tema/Tujuan Pembelajaran/Impor PROMES + collapse repeated "Detail"), `components/admin/dossier-section.tsx` (jump-nav `hidden lg:block`), `app/admin/(hr)/employees/[id]/page.tsx` (DetailRail hidden below lg — every card duplicated a section; students/guardians/classes already branch on `isMobile`). Review: fade sat under the sticky column → gated on no-sticky-column; reset guard ignored `hasExternalFilter` → fixed; `no-scrollbar` utility was undefined in this repo → arbitrary scrollbar classes.
- Task 2: primitives — new `components/ui/date-picker.tsx` (Calendar+Popover on fine pointer, native date input on `pointer: coarse` via new `hooks/use-coarse-pointer.ts`; local-time YYYY-MM-DD), `components/ui/rupiah-input.tsx` (Rp addon, id-ID separators while typing, integer|null out), `components/ui/async-combobox.tsx` (debounced, abortable, grouped, clearable; `footer`/`getGroup`/`getTriggerLabel` slots); `components/admin/{parent,student,class-section}-picker.tsx` rebuilt on it with unchanged props (`ClassSectionMultiPicker` stays hand-rolled — multi-select does not fit a single-value primitive); deleted `components/ui/form-field.tsx`, `components/ui/combobox.tsx` and the orphaned form-field test (0 importers). Review: campus group order regressed to first-seen → sorted; retry controller never aborted → shared abort ref; whitespace query fired a fetch → trimmed.
