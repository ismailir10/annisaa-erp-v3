...
10. [x] **.claude/skills/ship/SKILL.md**: add Step 1c — soft-skip + DEMO_MODE-skip delta check vs `origin/staging` baseline.
...
- Task 10: added Step 1c "Soft-skip + DEMO_MODE-skip delta check" to `.claude/skills/ship/SKILL.md` between 1b (full gate re-run) and Step 2 (open PR). The check counts `test.skip(true,` / `it.skip(true,` / `describe.skip(` / `process.env.DEMO_MODE === "true"` occurrences across test files (`:(glob)**/*.test.ts`, `*.test.tsx`, `*.spec.ts`, `*.spec.tsx` pathspecs) on `HEAD`, and the same count against `origin/staging` — both via `git grep -c` for methodological symmetry. Prints both + delta. **Existing skips grandfathered — only net growth (delta > 0) blocks `/ship`.** Reviewer pass (`feature-dev:code-reviewer`) caught a critical asymmetry in the first draft: had used filesystem `grep -r` on the current side + `git grep` on the baseline side, so files existing only in one tree would silently miscount, producing false-positive blocks; also caught that the source files containing `DEMO_MODE` branching (e.g. `lib/xendit/client.ts`) would inflate the count if pathspec didn't restrict to test files. Final symmetric form: both sides `git grep -cE` with `:(glob)**/*.{test,spec}.{ts,tsx}` pathspecs. Validated locally on this cycle: `baseline=35 current=26 delta=-9 PASS`. Block message lists three remediation paths plus a WIP-describe.skip carve-out per reviewer suggestion.
...
- **/ship** opened [PR #299](https://github.com/ismailir10/annisaa-erp-v3/pull/299) (`feat/slim-ship-gate` → `staging`).
- **Preview-verify skipped** per CLAUDE.md pure-docs rule: diff against `origin/staging` contains zero `app/**` pages, `components/**` UI, or `lib/**` business-logic — only `lib/__tests__/`, `app/api/__tests__/`, `e2e/`, `.claude/skills/`, `README.md`, `CLAUDE.md`, and the cycle doc. Nothing observable in a browser preview. CI's Playwright run is the canonical end-to-end gate; no Chrome MCP walk required.
