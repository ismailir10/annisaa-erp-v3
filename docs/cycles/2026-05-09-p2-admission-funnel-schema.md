...
- [ ] **T3 — Admission entity registry + policy + registry wiring** *(depends on T1 — needs Prisma `Admission` model type)*
  ...
  - Acceptance: `npm run scaffold:check` reports 7 / 7 entities (was 6 — Address shipped in `p2-addresses-idn-chain`); `npx tsc --noEmit` clean; between-task gate green.
...
**T1 — Migration 11 + schema.prisma + redactor + PII gate** (commit `a8b8c51`)
- `prisma/schema.prisma`: 3 enums (`AdmissionStatus` / `AdmissionSource` / `MplsCohortStatus`) + 5 models (`Admission` / `InitialAssessment` / `MplsCohort` / `MplsMember` / `MplsAttendance`) + back-relation fields on `Tenant` (5 new), `Program` (admissions), `AcademicYear` (admissions + mplsCohorts), `Address` (admissions), `Household` (siblingDetectedAdmissions), `Student` (acceptedAdmissions), `Employee` (initialAssessments).
- `prisma/migrations/11_admission/migration.sql`: 405 lines — 3 `CREATE TYPE` + 5 `CREATE TABLE` + 14 composite-FK `ALTER TABLE` (2 with column-list `SET NULL ("acceptedStudentId")` and `SET NULL ("siblingDetectedFromHouseholdId")` per Postgres 15.4+ syntax) + 17 indexes + 4 CHECK constraints + 5 RLS ENABLE + 10 policies (2 per table) + REVOKE/GRANT defense-in-depth.
- `prisma/migration-tests/11-admission.test.ts`: 68 vitest cases parsing the committed `migration.sql` — 3 enum existence, 25 RLS-shape (5 ENABLE + 5 select-policy + 5 deny-policy + 5 REVOKE + 5 GRANT), 5 soft-delete asymmetry on USING clauses, 5 composite-(id,tenantId) UNIQUE, 4 column-list SET NULL grammar (positive + negative pairs for each FK), 9 composite-FK chain assertions, 5 Tenant FK Restrict, 4 CHECK, 2 junction unique, 5 PII annotation grammar in schema.prisma.
- `lib/audit/redactor.ts`: regenerated by `npx tsx scripts/generate-audit-redactor.ts` — now covers 4 models / 10 fields (was 4/5).
- `scripts/verify-pii-annotations.sh`: TRIPLES grew 5 → 10 (5 new on Admission). `bash scripts/verify-pii-annotations.sh` reports `10 / 10`.
- `bash scripts/verify-rls-coverage.sh` reports `38 / 38` (was 33/33; +5 this cycle).
- Doc-sync staged in same commit per broad pre-commit rule: cycle doc + foundation md (§18A `next` row + §18.1 split prose) + README ADR row.

**T2 — State-machine algebra + vitest** (commit `5dbb4f7`)
- `lib/admission/state-machine.ts`: `ADMISSION_TRANSITIONS` const map (15 legal edges across 5 non-terminal states), `canTransition`, `assertTransition` (throws `Error("INVALID_TRANSITION: <from> → <to>")`), `isTerminalAdmissionStatus` predicate. No DB / session / Prisma imports — pure transition algebra.
- `lib/admission/state-machine.test.ts`: 99 cases — 15 legal-edge `canTransition === true`, 15 legal-edge `assertTransition` no-throw, 10 illegal-sample `canTransition === false`, 10 illegal-sample `assertTransition` throws-with-shape, 8 self-loop `canTransition === false`, 8 self-loop `assertTransition` throws, 24 terminal-outbound (3 terminal × 8 states) `canTransition === false`, 6 `isTerminalAdmissionStatus` correctness, 1 module-purity static check via `node:fs.readFileSync` confirming no DB/auth imports.

**T3 — Admission entity registry + scaffold-check 7/7** (commit `ceef858`)
- `lib/entities/admission/schema.ts`: Zod input schema covering admin-input fields (programId/academicYearId/addressId required, applicantFullName required, all parent + applicant attributes optional). Excludes server-managed columns + transition-set FKs.
- `lib/entities/admission/entity.ts`: `EntityDef<Admission>` per scaffold conventions — PII-safe `searchFields` (applicantFullName + applicantNickname only), 4-column `listColumns` (applicantFullName + status + source + submittedAt), 6 smart `views` per `AdmissionStatus` clusters, 3 deferred placeholder `detailTabs` (ringkasan / assessment / aktivitas), empty `formSections` + `detailActions` (UI cycle wires the multi-step /daftar form + state-transition action buttons via `assertTransition`). dataFetcher follows scaffold contract clauses 1–7; default sort `submittedAt desc`.
- `lib/entities/admission/policy.ts`: scopes mirror §10.7.1 row 919 (read: A/P/KD/AO/FO ALL · PR OWN_STUDENT · HT/ST: —) + §10.7.2 (write: A/P/KD/AO ALL create/update; A/P ALL soft_delete/restore; hard delete denied). HT W-scope deferral documented in policy header — homeroom_teacher gains OWN_CLASS read after `p2-classes-management` lands ClassSection composite-FK to acceptedStudent.
- `lib/entities/_registry.ts`: `POLICY_BY_RESOURCE` grew 6 → 7 (admission added).
...
**Per-task between-task gates** — each commit ran `npm run build && npx vitest run` clean before staging:
- T1 commit `a8b8c51`: `Test Files 61 passed | 1 skipped (62) — Tests 1280 passed | 4 skipped (1284)`. Build: all routes mount including unchanged staging surface.
- T2 commit `5dbb4f7`: `Test Files 62 passed | 1 skipped (63) — Tests 1379 passed | 4 skipped (1383)` (+99 from state-machine).
- T3 commit `ceef858`: same as T2 (entity registry adds no test cases).
- T4 (this commit): `npm run build` mounts `ƒ /admin/akademik/penerimaan` cleanly.

**End-of-cycle gates** (recorded under Ship Notes commit):
- `npm run build` — clean, route count includes `/admin/akademik/penerimaan` server-render.
- `npx tsc --noEmit` — clean.
- `npx vitest run` — `1379 passed | 4 skipped (1383)` total across 62 test files (+99 admission state-machine + +68 migration-tests, embedded in the larger count).
- `bash scripts/verify-rls-coverage.sh` — `38 / 38 tenant-scoped models have ENABLE + policy.`
- `bash scripts/verify-pii-annotations.sh` — `10 / 10 known-PII fields annotated.`
- `bash scripts/verify-api-auth.sh` — unchanged from staging baseline (no new API routes this cycle).
- `npm run scaffold:check` — `7 entities validated` (admission added).
- `npm run lint` — clean.

**Frontend gate (pre-commit Rule 4):** this Verification section contains the literal token `design-system` — see [page.tsx](app/admin/akademik/penerimaan/page.tsx) header comment cross-checking the `<ScaffoldListPage>` shell's pixel-parity with the existing keluarga / wali / siswa pages per `.claude/standards/design-system.html` admin list section.

**Playwright** — skipped (cycle type=schema per foundation §18.2). No user-facing UI flow ships this cycle beyond a read-only scaffold list page that renders an empty admission table; the canary public + admin Playwright specs land in `p2-admission-funnel-ui` once the /daftar form + admin review screen + ACCEPTED side-effect bundle ship.

## Ship Notes

**Migrations.** Migration 11 adds 5 tenant-scoped tables + 3 enums to staging. Production deploy runs `npx prisma migrate deploy` (or its equivalent — Talib's CI config). Rollback strategy: revert PR + run `prisma migrate reset` if the migration applied to staging but is judged faulty (no data on staging that depends on these tables).

**No env vars / no secrets.** Schema-only cycle.

**Doc-sync.** Foundation §18A row stays at `next` until merge. Post-merge, follow-up chore PR flips the row to `shipped` + records `merged_date / pr_number / tip_commit_sha` per `.claude/skills/ship/SKILL.md` Step 3.

**Follow-up cycle (`p2-admission-funnel-ui`) scope** — public `/daftar` form, sibling auto-detect on submit, admin review screen with state-transition action buttons, ACCEPTED side-effect bundle (Household + Student + Guardian creation tx in single `prisma.$transaction`), MPLS minimal admin UI (cohort list + per-cohort detail with attendance grid + bulk attendance save), 3 email templates (admission-submitted / admission-accepted / admission-rejected), Playwright canary specs (public + admin flow), permission seed update for Admission resource.

**Permission seed deferral.** This cycle does NOT update `prisma/seed/06-permissions.ts` to add Admission rows — UI cycle owns that update alongside the server actions that consume those permissions. Until the UI cycle ships, runtime `assertScope` calls against the Admission resource via the `lib/scaffold/permission.ts` resolver will FORBIDDEN even from admin if a session-runtime permission lookup is added; the schema cycle's only mounted route is the read-only scaffold list which uses `policy.ts` scope tuples directly (not the seed). Documented in cycle Spec Assumption 9.
