...
- [ ] **T7 — Submit transition server action.** Create `lib/admission/transitions/submit.ts` — wraps `assertTransition(DRAFT, SUBMITTED)` + `writeAuditLog` + `emitTimelineEvent("admission.status-changed")` + calls sibling-detect. `lib/admission/transitions/__tests__/submit.test.ts` covers happy path + illegal transition rejected + audit + timeline + sibling FK populated. AC: vitest green.
