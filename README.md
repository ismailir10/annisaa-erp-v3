...
| Date | Decision | Why |
|---|---|---|
| 2026-05-14 | Curriculum C5 (in flight) — sentra (CENTER) daily assessment on top of C4's `AssessmentEntry`; mobile UI at `/teacher/assessments/center/[center]` + POST/GET sentra session APIs (≤80 entries/session, ageGroup-filtered roster, design-system §forms compliant) | Reuses C4 schema with `source = CENTER` so 8 sentra rooms feed the same per-student data — see [cycle](docs/cycles/2026-05-14-curriculum-c5-center-assessments.md) |
