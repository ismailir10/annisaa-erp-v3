...
3. **Needs a DOKU Back Office login (blocked here — password required).** Two things to check while signed in: (a) **HTTP Notifications → Notifikasi** — is there any delivery attempt at all for the BCA payment, and what endpoint URL did it target? (b) **Settings → Payment Settings → Virtual Account SNAP** — does the SNAP variant carry its own notification URL, separate from the per-channel non-SNAP field? That would confirm or kill the SNAP hypothesis. The daily sweep means payments are credited regardless, so this is no longer silent-loss urgent — but a working webhook turns a day's delay into seconds, which matters a great deal to a parent who has just paid.
