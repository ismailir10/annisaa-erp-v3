...
6. [ ] **[P]** Fix F-12: remove `revalidate = 3600` from `app/api/attendance/monthly/route.ts`.
7. [ ] **[P]** Fix F-25: employee-creation advisory lock — use a unique numeric namespace constant exported from `lib/db.ts`. **Acceptance:** grep confirms `12345` no longer appears in employee or invoice subsystems with conflicting meaning.
