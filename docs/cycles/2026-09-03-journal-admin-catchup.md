...
- [x] **T5 — `app/admin/student-journal/page.tsx`.** Drop the page-level `px-page-x py-page-y`, keep `space-y-section`. *Acceptance:* no double inset vs. `layout.tsx`'s `<main>`; visually identical to the Monitoring/Attendance tabs' margins.
...
- T5: `app/admin/student-journal/page.tsx` — dropped the page-level `px-page-x py-page-y` wrapper class, kept `space-y-section`; `app/admin/layout.tsx`'s `<main>` already applies the same padding, so this page was double-inset relative to its Monitoring/Attendance siblings. Cross-checked `design-system.html` — no new spacing token, removal only.
- Code review: `feature-dev:code-reviewer` ran against the full combined diff (all 5 tasks) rather than per-task, given the tasks' small size and tight relatedness — found no high-confidence issues; specifically verified the `noteReloadToken` refetch wiring, the dossier card's read-only guarantee, no stale `.notes` references, and no weakened test assertions.
...
- `npm run build` — ✅ compiled successfully, all routes generated including the three touched admin pages.
- `npx vitest run` (full suite) — ✅ `Test Files 337 passed | 2 skipped (339)` · `Tests 3279 passed | 42 todo (3321)`.
- `bash scripts/verify-api-auth.sh` — ✅ `196 / 196` (unchanged — no route added/removed).
- `bash scripts/verify-rls-coverage.sh` — ✅ `42 / 42` (unchanged — no schema change).
- `bash scripts/audit-docs.sh` — ✅ 0 fail (counts block regenerated: 36 active cycle docs).
- **Playwright** — local run deferred to CI (this worktree's `.env` points at the shared staging Supabase pooler; `playwright.config.ts` refuses a non-local `DATABASE_URL` and the specs mutate data through the API). Required CI check `Playwright E2E` gates the merge.
- **Design system** — [x] Cross-checked `design-system.html` + `.claude/standards/ui.md`/`patterns.md` across every task: every component touched is an existing shared primitive (`NoteThreadPanel`, `WeekGrid`, `DetailPageHeader`, `DetailPageSkeleton`) used exactly as its existing callers (teacher/parent, or the other 6 admin detail pages) already use it — no new component, no new token, no markup invented.
- **Not verified this cycle** — live preview-verify (Chrome MCP) as the real admin account: no admin Google account is pre-authenticated in this session's Chrome profile (only teacher `ismail10rabbanii@` and parent `rightjet.hq@` per `.claude/verify-accounts.json`/prior session memory). `/ship` Step 3 will need `ismailir10@gmail.com` signed in to walk `/admin/student-journal/students/[id]`, the dossier's Buku Penghubung section, and `/admin/student-journal/classes/[id]` on the PR preview — flagging so the ship step doesn't silently skip it.

## Ship Notes

- **Migrations:** none. **Env vars:** none. **Routes:** none added, removed, or reshaped — this cycle only wires existing, already-authorized endpoints (`/api/student-journal/notes`, `/api/student-journal/notes/read`) into admin's UI.
- **Data:** none written beyond what `NoteThreadPanel` already writes for teacher/parent — a read watermark (`StudentJournalNoteRead`) upserted per admin reader per student on first view. No note content touched.
- **Behavior change worth announcing to admin staff:** the Catatan tab on `/admin/student-journal/students/[id]` and the Buku Penghubung card on the student dossier now show the student's whole note history, not just the viewed week — an admin who previously paged week-by-week to find an old note now sees it immediately.
- **Needs an admin account for preview-verify.** This session has no admin Google account pre-signed into Chrome — `/ship` Step 3 needs `ismailir10@gmail.com` (per `.claude/verify-accounts.json`) signed in before it can walk the admin flows on the PR preview. Flagging explicitly rather than skipping the check silently.
- **Rollback:** revert the merge commit. Nothing persisted changes shape; reverting restores the previous week-scoped admin note rendering with no data loss (the watermark rows written in the meantime are harmless and unused by the reverted code).
