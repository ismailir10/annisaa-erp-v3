...
- **CI typecheck fix-up commit (2026-05-19 22:57):** initial PR run flagged `lib/__tests__/assessment-entry-void.test.ts(16,38): TS2307: Cannot find module '@/lib/generated/prisma'`. Local `npm run build` had passed because Turbopack accepts a directory import; `tsc --noEmit` with `moduleResolution: bundler` is stricter and requires the explicit `/client` suffix that every other repo file already uses (see `lib/audit.ts`, `lib/db.ts`, etc.). One-character path fix; `npm run typecheck` ✓; focused vitest 1/1 still passes. Pushed as `fix(curriculum): C7a — tsc-strict path on void-cols type witness`.
