...
### T6 — Teacher student week + notes (done)

- `GET /api/student-journal/students/[id]/week?weekStart=`: TEACHER-only endpoint. Auth flow: role check → `tenantId` + `employeeId` required → `StudentEnrollment.findFirst({ status: "ACTIVE" })` → 404 if unenrolled → `TeachingAssignment.findFirst` with `classSection.tenantId` cross-tenant guard → 403 if unassigned. Validates optional `weekStart` YYYY-MM-DD param (400 if malformed), defaults to current Monday via `weekStart(today)`. Parallel fetch: SCHOOL categories (with active indicators), SCHOOL entries for the week range, notes for the week range (status=ACTIVE, desc). Returns `{ weekStart, dates, categories, entries, notes }`.
- `POST /api/student-journal/notes`: Rate-limited (20/60s per IP). Zod-parses `noteBodySchema`. TEACHER path: repeats enrollment + assignment guard (same chain as week route). GUARDIAN path: delegates to `requireGuardianForStudent`. Other roles: 403. Creates `StudentJournalNote` with `authorUserId: session.id`, `authorRole: session.role`. Returns 201 + note.
- `PUT /api/student-journal/notes/[id]`: Zod-parses `noteUpdateSchema`. Finds existing note; returns 404 if missing or `note.tenantId !== session.tenantId` (cross-tenant safety). Returns 403 if `note.authorUserId !== session.id` (author-only edit). Updates `body`. No rate limit (author-only, low risk).
- `app/teacher/student-journal/students/[id]/page.tsx`: Client component, `max-w-md mx-auto`. `useParams<{ id }>()` for student id. Week prev/next chevron navigation with `addDays(ws, ±7)` and a "Minggu ini / MM DD – MM DD" label. Fetches `/api/student-journal/students/[id]/week?weekStart=<ws>`, shows `Skeleton` while loading. Renders `<WeekGrid>` (read-only — no `editable` prop). "Catatan" section with "Tambah Catatan" `Dialog` button; dialog fields: Tanggal (date input, max=today) + Isi (Textarea 4 rows, maxLength 2000). On save: POST note → `toast.success("Catatan tersimpan")` → close dialog → clear fields → `loadWeek(ws)`. Below dialog: `<NoteThread notes={data.notes} />`. Back button uses `router.back()`.
