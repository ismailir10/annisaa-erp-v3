...
- **Parent Portal Visual Overhaul — cycle 3 (2026-04-23)**: full-portal visual craft pass across all 5 parent surfaces (home / invoices / attendance / reports / student-journal polish). New shared primitives `<SummaryHero>` + `<CardListItem>` (each 2nd-instance trigger met across 4 targets). StatusBadge extended with `variant='intent'` + Lucide icons + correctness fixes (text-[10px] → text-xs cycle-2 D3 miss; SICK tone red → amber; ABSENT default label "Tidak Hadir" → "Alpa" per voice.md glossary). EmptyState extended with `accent='warm'|'celebration'` (soft teal + gold tints for Islamic-courtesy moments). Tokens added: celebration-gold triad, shadow-card resting/elevated pair, motif-opacity. DataTable replaced with SummaryHero + CardListItem on /parent/invoices and /parent/attendance (right primitive for ≤5-item parent use case). Parent home greeting gets text-display ramp + subtle Islamic dot-lattice motif at 3% opacity. WeekGrid gets today-column teal bracket + category-header warm left-accent + Jumat-no-clip at 375px — see [`docs/cycles/2026-04-23-parent-visual-overhaul.md`](docs/cycles/2026-04-23-parent-visual-overhaul.md)
...
- **Admin Polish — Card Padding Sweep (2026-04-23)**: 12 `<Card className="p-5 …">` sites across 6 admin pages (employees/[id], dashboard-client, students/[id], settings/campuses, invoices/[id]) swapped to the `p-card` token (1.5rem). Real visual delta: +4px padding per side (`p-5` = 1.25rem → `p-card` = 1.5rem). Re-audit after shipping the Quick Fixes Sweep revealed planned Cycles 2+3 (assessments/monthly STATUS_COLORS, student-detail stat-card semantic tokens, audit-log inline colors, simple Skeleton shape) were false positives — all already use legitimate semantic tokens per colors.md. See [`docs/cycles/2026-04-23-admin-polish-card-padding.md`](docs/cycles/2026-04-23-admin-polish-card-padding.md)
- **Admin Polish — StatsCardsRow + Final Residual Sweep (2026-04-23)**: (1) 9 inline stat-card grids migrated to `<StatsCardsRow cols={3|4}>` (guardians, admissions, payroll/[id], attendance, enrollments, assessments, student-attendance, student-journal/monitoring, assessments/templates). (2) Route-level create forms migrated to in-list Dialog/Sheet: `students/new` deleted (orphaned), `employees/new` + `payroll/new` collapsed into `employees/page.tsx` and `payroll/page.tsx` with `useIsMobile()` Dialog-or-Sheet switching, form-reset on close, and dashboard quick-actions now use `?create=1` auto-open pattern. (3) Assessments `BB` grade chip remapped off absent-red to late-amber per PAUD pedagogy (Belum Berkembang is developmentally normal; red conflates with "absent" on attendance). MB→blue, BSH→purple, BSB→green. (4) `/build` skill HEREDOC dedupe — manual `Co-Authored-By` line dropped so `prepare-commit-msg` hook is the single trailer source. Playwright e2e test updated off the deleted route. See [`docs/cycles/2026-04-23-admin-polish-stats-cards-row.md`](docs/cycles/2026-04-23-admin-polish-stats-cards-row.md)
- **Admin Polish — Quick Fixes Sweep (2026-04-23)**: residual design-system drift swept across admin pages — `bg-primary/10` icon halos → `bg-muted` (9 sites: layout, settings/campuses, settings/users, attendance, employees, dashboard, payroll/[id], students, invoices); assessments grade chips (BB/MB/BSH/BSB) swapped from `bg-red-100`/`orange-100`/`blue-100`/`green-100` arbitrary palette to `bg-status-*-subtle` tokens; `text-[9px]` → `text-caption` (payroll Adj marker + dashboard day label); `font-currency` stripped from 5 non-numeric/non-ID cells (dates/sort-order/time/count), with `tabular-nums` kept where column alignment matters. Task 5 (`p-card` dedupe) canceled — investigation showed `<Card>` has no default horizontal padding, so `p-card` is load-bearing not redundant. Follow-up cycles (2–6) tracked in the cycle doc. See [`docs/cycles/2026-04-23-admin-polish-quick-fixes.md`](docs/cycles/2026-04-23-admin-polish-quick-fixes.md)
