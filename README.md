...
- **Student Journal — teacher entry UI (2026-04-21, T5 done)**: mobile-first picker page (class + date) + class-day entry grid (`ClassDayGrid` component with expandable per-student cards, 44px indicator buttons), sticky save bar, "Penghubung" tab added to teacher bottom nav, Buku Penghubung quick-link tile on teacher home — see [`docs/cycles/2026-04-21-student-journal.md`](docs/cycles/2026-04-21-student-journal.md)
- **Student Journal — parent portal (2026-04-21, T7-T8 done)**: `requireGuardianForStudent` guard (User→Parent→StudentGuardian chain), `GET /api/student-journal/children/[id]/week` (school+home categories, entries split by scope, notes), `GET /api/parent/children`, `POST /api/student-journal/entries/home` (HOME-scope upsert, guardian-verified). Parent portal `/parent/student-journal` with child selector, week picker, Di Sekolah read-only grid, Di Rumah editable grid (optional, no nag), Catatan thread. Bottom-nav gains "Penghubung" tab — see [`docs/cycles/2026-04-21-student-journal.md`](docs/cycles/2026-04-21-student-journal.md)
- **Student Journal — teacher student week view + notes (2026-04-21, T6 done)**: `GET /api/student-journal/students/[id]/week` (teacher-scoped, enrollment+assignment guard), `POST /api/student-journal/notes` (TEACHER or GUARDIAN, rate-limited), `PUT /api/student-journal/notes/[id]` (author-only edit, cross-tenant 404). Teacher page `/teacher/student-journal/students/[id]` with week prev/next nav, read-only `WeekGrid`, add-note dialog, `NoteThread` — see [`docs/cycles/2026-04-21-student-journal.md`](docs/cycles/2026-04-21-student-journal.md)
