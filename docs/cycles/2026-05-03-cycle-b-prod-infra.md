...
- **Hotfix on PR #169 (post-initial-push):** CI Playwright failed with `users.find is not a function` on 4 specs. Root cause: T4 rate limit on `/api/auth/*` (5 req/min/IP) caught `/api/auth/users` — the `@public` demo-only user-selector route. E2e test setup calls it once per file in `beforeAll`; multiple files × 2 retries exceeded 5/min, returning 429 + `{error:"rate_limited"}` instead of the expected user array. Fix: skip auth rate limit entirely when `DEMO_MODE=true` (one early-return in `lib/security/auth-rate-limit.ts`). Demo mode bypasses Supabase Auth anyway via proxy.ts, so the rate limit was never the protection layer for demo flows; production unchanged (DEMO_MODE unset → check runs as before). Both reviewers cleared the fix; new unit test (20 successive calls pass under DEMO_MODE) prevents regression. PR re-pushed; CI re-ran green.
