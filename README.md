...
| Date | Decision | Why |
|---|---|---|
| 2026-05-18 | Kesiswaan CRUD parity (in flight): superset parent-option constants; auth-proxied Student photo upload; metadata key/value editor; "Riwayat Status" sub-card; Student create dialog full field set; Admission `campusPreference` end-to-end (validation + PUT + POST + admin Select sourced from `/api/config/campuses`) | KTP/KK (T14) needs the storage adapter; admins had to round-trip via detail page for non-core fields; `campusPreference` was schema-present but silently dropped on update — see [cycle](docs/cycles/2026-05-18-kesiswaan-crud-audit.md) |
