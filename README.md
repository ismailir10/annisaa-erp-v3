...
| Date | Decision | Why |
|---|---|---|
| 2026-05-18 | Kesiswaan CRUD parity (in flight): superset parent-option constants; auth-proxied Student photo upload; metadata editor; "Riwayat Status" sub-card; Student create dialog full field set; Admission `campusPreference` end-to-end; Parent KTP + KK admin-only upload via `lib/storage/parent-document` factory (5 MB, JPG/PNG/PDF magic-byte, never under `public/`) | Closes UU PDP 27/2022 compliance gap for sensitive PII docs — see [cycle](docs/cycles/2026-05-18-kesiswaan-crud-audit.md) |
