...
| **students** | Student lifecycle: Household, Student, StudentIdentifier (NIS history), StudentIdentifierSequence (NIS allocator anchor), Guardian (deferred to p2-guardians), enrollments, admissions |
...
| 2026-05-06 | v2 Google OAuth callback + auth surface — `/auth/callback` PKCE exchange + email-based User resolve + CAS supabaseUserId backfill + 3-layer `?next=` validation; `lib/auth/{callback-origin,demo-cookie,session}` (NEXT_PUBLIC_SITE_URL pin, HMAC demo cookie, demo+Supabase resolver); `/api/_demo/login` DEMO_MODE-404; `/auth/error` page; JWT-hook dashboard runbook | Closes Phase 1 final cycle (auth was the last `getSession()` deferral). Phase 1 foundation truly DONE. p2 entity cycles unblock — see [cycle](docs/cycles/2026-05-06-p1-auth-google-oauth.md) |
