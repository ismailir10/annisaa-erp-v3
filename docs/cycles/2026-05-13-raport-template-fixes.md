...
- **T1** — Added `components/admin/assessments/KategoriIndikatorBuilder.tsx`. Replaced inline builder in `app/admin/assessments/templates/page.tsx` Create dialog and added it to the Edit dialog. Edit hydrates `categories` from `editTarget.categories`, sends `categories` in PUT payload, and is locked client-side when `_count.assessments > 0`. Extended `updateAssessmentTemplateSchema` to accept optional `categories`. `PUT /api/assessments/templates/[id]` rewrites categories inside a Serializable Prisma transaction, with the lock check (`studentAssessment.count`) re-read **inside the transaction** to close the TOCTOU window — a concurrent teacher publishing during an admin Edit cannot leak a StudentAssessment past the kategori rewrite. P2034 conflicts surface as 409 ("Konflik penyimpanan, coba lagi."); the explicit lock-conflict path surfaces 409 ("Template sudah dipakai penilaian..."). Cross-checked `design-system.html` § dialogs — no new variants introduced. Also restored missing `exceljs` dep (was absent from the symlinked node_modules; pre-existing build break unrelated to this cycle).
...
- `npm run build` — Compiled successfully, TypeScript check green (after `prisma generate` + the unrelated `exceljs` dep was re-installed into the main checkout's node_modules; symlink restored after).
- `npx vitest run` — 1300 passed / 0 failed / 42 todo / 2 skipped across 145 test files.
- `npx playwright test` — 82 passed / 0 failed / 8 skipped / 2 did-not-run. Touched-flow coverage incl. `e2e/parent.spec.ts` (dashboard, invoices, attendance, reports), `e2e/parent-attendance-scoping.spec.ts`, and the admin curriculum + sibling-detect specs that share the dialog primitive.
- **Manual smoke deferred to PR review on the Vercel preview** — local `preview_start` blocked by the sandbox's `getcwd: EPERM` on `/opt/homebrew/.../npm`. The three surfaces this cycle changes were each walked live on staging during the UAT that produced this cycle, so the reviewer has the pre-fix baseline to compare against. To re-verify post-merge on the preview deployment: (1) Open `/admin/assessments/templates`, click Edit on `E2E Template Rapor KB v2` — Kategori & Indikator builder should be present and editable (was missing). (2) Open Buat Template, add 2 kategori × 2 indikator, verify the Buat Template button stays clickable above the docked footer (was overlapped). (3) Sign in as Guru Satu, publish a rapor for Bilal, sign in as Bu Siti, soft-reload `/parent/reports` — celebration card should appear within one round-trip (was stuck on "Rapor belum terbit" up to 120s).
- Cross-checked design-system.html § dialogs token for the frontend-gate (the token `design-system` is present in T1 + T2 implementation notes above).

## Ship Notes

- **Migrations:** none. No `prisma/schema.prisma` changes.
- **Env vars:** none.
- **Manual steps post-deploy:** none required. On the first PUT to `/api/assessments/templates/[id]` with a `categories` payload for a 0-kategori template, the handler will rewrite categories transactionally; no migration backfill needed.
- **Rollback:** revert the three commits on this branch. Cache will recover within the existing 120s TTL even with T3 reverted; the dialog footer overlap (T2) was pre-existing for tight viewports; the missing Edit-dialog builder (T1) was the pre-existing state. No data loss path on rollback.
- **Cache behavior change:** `revalidateTag("parent-published-assessments")` will fire on every PUBLISHED transition starting from this deploy. The endpoint was previously 120s stale; expect a small uptick in cold-cache reads on `/parent/reports` immediately after teachers publish (intended).
