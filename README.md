...
| 2026-05-12 | Private `admission-documents` Supabase Storage bucket; uploads/downloads via signed URLs only (deny-anon RLS); `lib/supabase/service-client.ts` singleton; `lib/supabase/storage.ts` typed SDK wrapper; `POST /api/public/admissions/upload-token` — rate-limited signed-upload-URL endpoint (status guard: INQUIRY/VISITED/APPLIED only) | Admission doc uploads (KTP Ayah/Ibu, KK) need presigned paths; direct anon reads blocked — see [cycle](docs/cycles/2026-05-12-admission-storage.md) |
