...
- [x] **T7 — Enrich seed planners (full-field + dedup).** Update `scripts/reseed/people.ts`:
...
- Task 8: README + Ship Notes — `README.md` "Reseeding staging" section now lists `XENDIT_WEBHOOK_TOKEN` in the env-var table (must be set in Vercel preview env, otherwise webhook handler 401s every delivery) and includes a 6-step "Post-reseed UAT smoke" walkthrough for the parent payment flow + a `WebhookEvent` SQL inspection snippet for triage. Cycle doc Ship Notes filled with: migration auto-applies on Vercel build, `XENDIT_WEBHOOK_TOKEN` env requirement, 5-step operator post-merge clean-reseed sequence, two follow-ups (90-day retention; attemptCount-based retry-cap), rollback path.
- Task 7: Enrich seed planners + name dedup — `scripts/reseed/names.ts` gained 7 new pools (BIRTH_PLACES, BLOOD_TYPES, HOBBIES, ALLERGIES, BEKASI_ADDRESSES, EMPLOYERS, EMPLOYER_CITIES) + BANKS_WEIGHTED + `pickWeightedBank`. `scripts/reseed/people.ts` extended StudentPlan/ParentPlan/EmployeePlan types with all schema-allowed non-required fields; added `synthNis/Nisn/Nik/Kk/BankAccount` helpers (synthetic ids prefixed with `9`/`99` to mark obviously fake) and exported `uniqueName(seen, genFn, max)` retry-then-suffix-fallback helper. `planStudents` registers preserved children first then uses `uniqueName` for synthetic dedup; same pattern for `planParents` (display name) and `planEmployees` (nama). `seedPeople` writer passes the new fields to `prisma.{student,parent,employee}.create`. 13 new vitest cases covering uniqueName logic, full-field assertions on each plan, no-duplicate-name across full cohort. Reviewer caught uniqueName suffix-fallback bug — fresh draw could land unseen but code still appended suffix; fixed with explicit `if (!seen.has(base))` short-circuit before suffix loop.
...
- Task 7: `npx vitest run scripts/reseed/__tests__/people.test.ts` — 25/25 passing (+13 cases). Full suite 494 passed / 42 todo / 2 skipped across 63 files. `npm run build` — clean. Reviewer caught uniqueName suffix-edge bug; fixed.
- Task 8: `npm run build` clean. Full `npx vitest run` 494/494 passing (494 passed / 42 todo / 2 skipped). `npx playwright test` — 33 admin/teacher specs failed locally for the same environmental reason recorded in PR #137: local `.env` `DATABASE_URL` points at the freshly reseeded staging Supabase whose 200-student/28-employee shape diverges from the demo SQLite skeleton the e2e tests assume. CI runs Playwright against the demo-mode build (no `.env.staging`) and is expected green; this cycle changes only seed scripts + one API route + Prisma schema + the webhook test file — no `app/admin/**`, `app/teacher/**`, `app/parent/**`, `components/**`, or `lib/format.ts` touched, so demo-mode page rendering is identical to PR #137 baseline. Cross-checked design-system.html: not applicable — infrastructure cycle, no frontend.

## Ship Notes

**Migrations:** new — `prisma/migrations/20260425100000_add_webhook_event_table/migration.sql`. Creates `WebhookEvent` table + 3 indexes + RLS deny-all. Applies automatically on Vercel build for `staging` and `main` branches via `scripts/vercel-build.sh` `prisma migrate deploy`.

**New env vars:** `XENDIT_WEBHOOK_TOKEN` (Vercel preview env). Without it, webhook handler 401s every delivery. Operator must verify it is set in Vercel before merging this PR.

**Operator post-merge sequence (clean reseed):**
1. Take a Supabase snapshot via dashboard → Database → Backups.
2. Verify Xendit dashboard webhook URL points at the staging preview domain and `XENDIT_WEBHOOK_TOKEN` matches Vercel.
3. Refresh `.env.staging`: `npx vercel env pull .env.staging --environment=preview --git-branch=staging`.
4. Run **clean reseed (full destructive wipe + reseed)**: `STAGING_CONFIRM=yes npm run reseed:staging`. The script wipes every application row, recreates the 6 preserved auth users, regenerates 200 students / 200 parents / 28 employees with the new full-field schema, and creates 540 fresh Xendit sandbox sessions whose `reference_id = invoice.id` (T1 fix). Reseed-time smoke assert (T2) throws if the round-trip is broken.
5. Smoke-test the parent payment flow per the README "Post-reseed UAT smoke" section.

**Follow-ups (separate cycles):**
- WebhookEvent 90-day retention job (cron route or `pg_cron`) to delete `status IN ('PROCESSED','IGNORED') AND createdAt < now() - interval '90 days'`. Captured for future cycle.
- DELETE-on-throw retry-cap hardening: add an `attemptCount` column to `WebhookEvent`, increment instead of DELETE on failure, 200 with `FAILED:max_attempts` after N attempts. Captured for future cycle. Xendit's ~5× retry cap is acceptable mitigation for now (staging only).

**Rollback:** revert the merge. Migration is forward-only; if rollback is permanent the operator should manually `DROP TABLE "WebhookEvent"` in staging via Supabase SQL editor. The reseed scripts keep their own snapshot-based rollback path (Supabase dashboard backup → restore).
