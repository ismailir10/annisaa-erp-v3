...
| 2026-04-24 | RLS enforces **tenant-scoped SELECT only**; INSERT/UPDATE/DELETE go through `service_role` (Prisma bypasses RLS). App-layer `tenantId` filtering in route handlers is the real write-side isolation — **a leaked service_role key bypasses RLS entirely.** `scripts/verify-rls-coverage.sh` is wired into CI to fail if any tenant-scoped model lacks an ENABLE + policy migration. Follow-ups tracked: (a) recreate 37 tenantId indexes dropped by `20260421000001_rls_security_cleanup` before multi-tenant scale; (b) review CASCADE on `EmailLog.tenantId` + `OrgConfig.tenantId` before onboarding a second tenant (tenant deletion wipes audit trail). See [`docs/cycles/2026-04-24-stress-review-fixes-2.md`](docs/cycles/2026-04-24-stress-review-fixes-2.md) |
