...
- CI follow-up: lint rejected setState in the RecapView effect body (hoisted fetch into a useCallback); e2e strict-mode on duplicate "Kelas Tujuan" text (scoped `.first()`); pre-existing `bulk-promote-race.test.ts` asserted `FOR UPDATE OF cs` (updated to plain `FOR UPDATE` + no-`GROUP BY` after the 0A000 fix). All three pushed; CI Build + Playwright green, unit suite 1970 passed.
