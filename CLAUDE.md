...
>
> **Migrations landed (Phase 1):** `00_extensions` (pg_trgm + pgcrypto), `01_tenancy` (Tenant/Campus/Program/AcademicYear/AcademicTerm), `02_identity` (User/Role/Permission/UserRole/RolePermission + composite FKs + RLS retroactive coverage on tenancy tables + Supabase Custom Access Token Hook), `03_employees` (Employee + EmployeeCampusAssignment + Campus(id, tenantId) backfill + Program.headEmployeeId FK wire-up), `04_classes` (ClassSection + Sentra catalog + TeachingDefault + SentraRotation + Program/AcademicYear/AcademicTerm composite-unique backfills + dayOfWeek CHECK on SentraRotation), `05_sessions` (ClassSession + SessionTeacher + SessionStatus + SessionTeacherRole enums + single-PRIMARY-per-session partial-unique guard + dayOfWeek CHECK on ClassSession), `09_regions` (Province/Regency/District/Village + RegencyType enum + trigram GIN on Village.name + public-read RLS — `idn-area-data` v4.0.1 vendored snapshot, ~91.6k rows applied via `prisma/seed/01-regions.sql`; non-tenant-scoped so it sits outside `verify-rls-coverage.sh`'s tenant set). **Seeds landed:** `00-tenant`, `01-regions.sql`, `02-campuses`, `03-programs`, `04-academic-year`, `05-system-roles`, `06-permissions`, `07-sentra` (8 PAUD catalog rows).
>
> **RLS coverage guard:** `scripts/verify-rls-coverage.sh` runs in **strict** mode as of `p1-identity-rls` (2026-05-05) — every tenant-scoped Prisma model must have both `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` and at least one `CREATE POLICY` in `prisma/migrations/**`. Current count: **17 / 17** as of `p1-employees-classes-sentra` (8 new tables added 9 → 17). The mid-rebuild parser-regression floor is set to 5 (will pass-through naturally as remaining Phase 1 + Phase 2 cycles push the count past 20).
