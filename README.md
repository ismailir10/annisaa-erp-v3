...
| Date | Decision | Why |
|---|---|---|
| 2026-05-18 | Kesiswaan CRUD parity (in flight): superset parent-option constants; Student photo + Parent KTP/KK auth-proxied uploads; Student-metadata editor; lifecycle "Riwayat Status" sub-card; Student create dialog full field set; Admission `campusPreference` end-to-end; unified `GuardianFormBody`; race-safe `isPrimary` + `childOrder` editable under serializable tx | Closes UU PDP 27/2022 compliance gap; eliminates 3-way guardian-form drift; concurrent primary toggle no longer permits two-primary state — see [cycle](docs/cycles/2026-05-18-kesiswaan-crud-audit.md) |
