...
- Playwright: attempted `npx playwright test` — confirmed setup-time refusal, not a test failure: `playwright.config.ts:40` (`assertLocalDatabaseForE2E`) hard-blocks any run whose `DATABASE_URL` host isn't local (`"Refusing to run e2e against non-local DATABASE_URL host aws-1-ap-southeast-1.pooler.supabase.com..."`), and this worktree's `.env` (symlinked from the main checkout) points at the real staging Supabase project. No local override available without setting `E2E_ALLOW_REMOTE_DB=1` against real staging data, which this session will not do. Deferred to CI, which runs against the Vercel preview's own isolated env. Required CI check `Playwright E2E` gates the merge; CTO will not merge on red.

- Preview-verify iteration 1 (https://annisaa-erp-v3-git-feat-salary-dee0ed-ismails-projects-196d40d3.vercel.app): flows=[admin payroll detail (APPROVED run) — "Kirim Slip" gone / "Ekspor BSI" present / no console errors; teacher /teacher/slips regression check — Mar/Feb/Jan slips all "Tersedia" with PDF download, unaffected by the email-path removal; admin admissions "Kirim Formulir" on a Diterima-status admission — response `sent:true`, `formUrl` host resolves to the preview's own domain (`annisaa-erp-v3-git-feat-salary-dee0ed-ismails-projects-196d40d3.vercel.app`), not `talib.annisaasekolahku.com`, confirming the resolveAppOrigin fix works end-to-end on a live non-prod deployment], blockers=0, minors=0.
- Preview-verify converged on iteration 1 (clean): 1 iteration, 0 fix commits, final preview https://annisaa-erp-v3-git-feat-salary-dee0ed-ismails-projects-196d40d3.vercel.app.
