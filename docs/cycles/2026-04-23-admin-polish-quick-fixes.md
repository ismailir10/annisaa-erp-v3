...
- **Task 4** — `font-currency` misuse removal. `font-currency` = mono + tabular-nums utility intended for currency amounts + numeric IDs (NIS/NIK/account numbers). Stripped from 3 non-numeric / non-ID sites: `settings/holidays:103` (date cell — dropped fully), `settings/salary-components:111` (single-digit sort order — dropped), `payroll/page.tsx:76` ("N hari" count — replaced with `tabular-nums`). Attendance check-in/check-out time columns (`attendance:116` + `:126`) were initially stripped but **restored** after code review pointed out `design-system.html:2154-2155` explicitly prescribes `font-currency` for small time displays (Masuk/Pulang); `tabular-nums` alone aligns less crisply than the mono variant that peer cells (e.g. `employees/[id]` time ranges) retain. Restore landed in the post-review amend commit. All other `font-currency` sites (currency amounts, NIS/NIK/account IDs) left untouched per spec scope.
- **Task 5** — canceled, not skipped. Spec Assumption #4 claimed `<Card>` default padding covered `p-card`. Investigation showed `components/ui/card.tsx:15` only sets `py-4` — horizontal padding lives on `CardHeader`/`CardContent` via `px-4`. `settings/config/page.tsx:85` uses `<Card>` directly without sub-components, so `p-card` is load-bearing (not redundant). Removing it would strip all horizontal padding. No change shipped. Spec acceptance bullet reconciled to `- [~]` with rationale.
...
End-of-cycle Playwright: `npx playwright test` → **27 passed, 2 skipped, 11 failed** (4.3m). Full-suite reproduction on pre-cycle HEAD (`git checkout 03e0c1f -- . && npx playwright test`) → **identical 27 passed / 11 failed / 2 skipped** with the exact same 11 failing test IDs. All failures confirmed pre-existing, not cycle-induced. Root cause is a pre-existing environment issue in this harness-provided worktree (auth cookie / demo-mode seed); tracked separately.
...
- **New follow-up items from code review (2026-04-23):**
  - **Review BB → `status-absent-subtle` semantic fit.** In PAUD pedagogy "Belum Berkembang" is developmentally normal for young children; rendering it in the same red as attendance-absent may conflate a benign tier with a negative event. Options for a future cycle: (a) add dedicated `status-bb/mb/bsh/bsb-subtle` tokens with softened hues, or (b) remap BB → `status-late-subtle` (amber) and shift the scale up so BSB still lands on `status-present-subtle`. Decide before the chips leak to any parent-facing surface. Severity: design clarity, not functional regression — the pre-cycle code already used `bg-red-100` for BB, so this cycle didn't worsen the semantic mismatch.
  - **Commit-message template dedupe.** All 4 task commits on this cycle ended up with two `Co-Authored-By` trailers (one added by the `/build` HEREDOC, one appended by `.githooks/prepare-commit-msg`). Drop the manual `Co-Authored-By: Claude Opus 4.7 (1M context) …` line from the `/build` skill's commit template so the hook is the single source of truth. Cycle-internal fix only — no amend of already-landed commits.
