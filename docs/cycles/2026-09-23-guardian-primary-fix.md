...
- Task T5: gates passed (wave-2 run above). Proof it bites: with step 1 temporarily sending `isPrimary: false`, the positive test failed — `AssertionError: expected { name: 'Siti Aminah', …(16) } to not have property "isPrimary"` / `Tests 1 failed | 2 passed (3)` — then restored. The negative control independently proves the downstream chain discriminates (`parentId` null, `customerName` = student).
