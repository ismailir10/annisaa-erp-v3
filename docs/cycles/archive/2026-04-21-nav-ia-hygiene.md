...
- **CI fix-up (post-PR):** PR #85 lint job flagged `react-hooks/set-state-in-effect` on the new sidebar useEffect. Suppressed with a scoped `eslint-disable` carrying justification — the effect syncs URL pathname (external system) into local collapsible state, and the functional setState bails out when the value matches, so no cascading renders. Locally `npx eslint components/admin/sidebar.tsx` is clean.
