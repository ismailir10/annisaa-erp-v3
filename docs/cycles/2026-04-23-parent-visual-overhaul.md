...
- **Task S1 (StatusBadge severity + glossary + icon variant):** `components/ui/status-badge.tsx` rewritten (+~130 lines). Additive props: `variant?: 'solid' | 'intent'` (default `'solid'` = byte-equivalent to cycle-2 rendering for 30+ existing consumers) and `icon?: LucideIcon`. New `STATUS_ICON_MAP` auto-selects a Lucide icon per status (PRESENT→Check, ABSENT→X, SICK→Thermometer, PUBLISHED→Sparkles, OVERDUE→AlertTriangle, …); new `STATUS_LEFT_BORDER_MAP` applies a 4 px tone-tinted left accent in intent mode. Three correctness fixes rolled into the default variant: (1) banned `text-[10px]` → `text-xs` per portal.md §Portal Text-Size Scale floor (cycle-2 miss); (2) `SICK` tone red→amber (`bg-status-absent-subtle` → `bg-status-late-subtle`) per design-system.html §Status palette — Sakit is warn-severity, not danger; (3) `ABSENT` default label `"Tidak Hadir"` → `"Alpa"` per voice.md glossary (consumers can override via `label` prop). `getStatusConfig` signature preserved; new `StatusBadgeProps` type exported. New test `components/ui/__tests__/status-badge.test.tsx` covers default label, intent icon auto-selection, icon override, SICK amber class, ABSENT "Alpa" default. Cross-checks: `.claude/standards/design-system.html §Status-chip set`, `.claude/standards/voice.md §glossary`, `.claude/standards/portal.md §Portal Text-Size Scale`. Gates: `npm run build` ✓, `npx vitest run` 273 pass (5 new), `npm run lint` 0 errors.
- **Task S2 (empty-state warm + celebration accent):** `components/ui/empty-state.tsx` adds optional `accent?: 'neutral' | 'warm' | 'celebration'` prop (default `'neutral'` — byte-equivalent to pre-S2 rendering so existing consumers keep current look). `warm` → `size-16 rounded-full bg-primary/10` + `text-primary size-8` icon, for inviting parent moments (e.g. no-invoices-yet, waiting-for-teacher-to-fill-attendance). `celebration` → `size-16 rounded-full bg-celebration-gold-subtle` + `text-celebration-gold-text size-8` icon with decorative `Sparkles` (lucide, `size-3.5 text-celebration-gold`, absolute `-top-1 -right-1`), for rapor-published + semua-lunas moments. Tokens only (consumes S4 `--celebration-gold*` triad); no new npm deps. Vitest at `components/ui/__tests__/empty-state.test.tsx` covers all three accents (neutral muted-circle, warm primary-tinted circle, celebration gold circle + Sparkles). `npm run build` ✓, `npx vitest run` 268 pass / 42 todo, `npm run lint` 0 errors. Cross-checked `.claude/standards/design-system.html` §Empty patterns + `.claude/standards/voice.md` "Empty states" + parent copy examples.
- **Task S5b (CardListItem primitive):** `components/portal/card-list-item.tsx` (new) + `components/portal/__tests__/card-list-item.test.tsx` (new, 6 tests). Tappable card-row primitive covering leading / primary / secondary / meta / trailing slots with three render forms (Next `<Link>` when `href`, `<button type="button">` when `onClick`, static `<div>` otherwise). Auto-appends `ChevronRight` on interactive forms only when the consumer omits `trailing` (consumer override honored). Press-state `active:scale-[0.98]` + 150 ms transform transition per design-system.html §13 overlays; hover tint `bg-muted/50`; focus-visible ring on all forms; `disabled` degrades to muted non-interactive div. **2nd-instance extraction trigger met at 4 consumers** (documented in file-header JSDoc): T1 HouseholdOverview child rows, T2 parent invoice rows (replaces DataTable for <10-row parent norm), T3 attendance-day rows (replaces DataTable), T4 rapor-term rows. Cross-checked `.claude/standards/design-system.html` §14 list patterns + §13 overlays (press-state rules) and `.claude/standards/portal.md` §Portal Primitive Inventory. `npm run build` ✓, `npx vitest run` 273 pass / 42 todo, `npm run lint` 0 new errors. No consumer wired yet — T-tasks do the swap.
- **Task T3b+T3c+T3d (parent-attendance page reorder + CardListItem day list + warm empty state):** `app/parent/attendance/page.tsx` reordered top-of-page per design-system.html §14: `ChildSelectorTabs → PageHeader("Kehadiran Anak", subtitle "Pantau kehadiran harian") → SummaryHero(week) → filters + day list`. Hero no longer inverts the H1 hierarchy. `app/parent/attendance/client.tsx` rewritten to drop the admin-grade `DataTable` in favor of a vertical `ul` of `CardListItem` rows — leading: `size-11 rounded-full bg-primary/5 text-primary` date circle with weekday short label (`Sen/Sel/Rab…`, `text-xs` per portal.md §Portal Text-Size Scale floor, `leading-none`) + day-of-month (`text-sm font-semibold leading-tight`); primary: `formatDate(date)` long Indonesian form (e.g. `21 April 2026`); secondary: teacher note or `"Tidak ada catatan"`; trailing: `<StatusBadge status={status} variant="intent" />` (S1 icon + tone + 4 px left accent, decipherable at 3 m glance); newest-first; no onClick (no note-dialog pattern exists in client.tsx yet — deliberately out of scope). Native `<input type="date">` pair replaced with Shadcn `<Input type="date">` wrapped in `<Field>` + `<FieldLabel>` so labels are visible to sighted parents (not just `aria-label`); status filter already used Shadcn `<Select>`, now also wrapped in `<Field>`. Empty branch renders `<EmptyState accent="warm" icon={Calendar} title="Belum ada catatan kehadiran" description="Insyaallah akan muncul setelah Ustadzah mengisi absensi hari ini." />` per portal.md §Empty State Contract; error + filtered-empty variants swap copy. Loading renders 5× `Skeleton h-[68px] rounded-xl` rows. Pagination simplified to Sebelumnya/Berikutnya buttons + `Halaman N dari M · K catatan` counter (DataTable page-size selector dropped — parent use case is <30 rows/month). API contract, URL params, fetch/debounce semantics preserved byte-for-byte. Minor TS fix: `handleStatusChange` accepts `string | null` to match Shadcn Select `onValueChange` signature. Gates: `npm run build` ✓, `npx vitest run` 273 pass / 42 todo, `npx eslint app/parent/attendance/` clean, `grep -rn 'text-\[10px\]\|text-\[11px\]' app/parent ...` zero matches. Visual verification deferred — local preview server blocked by `EPERM: uv_cwd` in this worktree; Playwright smoke will run at end-of-cycle gate.
- **Task T3a (parent-attendance weekly SummaryHero):** `app/parent/attendance/week-summary-strip.tsx` rewritten to consume `SummaryHero` primitive. Tone derived from worst-severity week status: `ABSENT > 0` → `danger`; else `SICK + PERMISSION > 0` → `warn`; else `PRESENT > 0` → `success`; empty week → `neutral`. `CalendarDays` icon, `elevated={false}` (secondary hero, H1+list own the primary focus), primary text `"Minggu ini: Hadir {N} · Sakit {N} · Alpa {N} · Izin {N}"` (keeps "Alpa" per voice.md glossary), secondary `"Ringkasan kehadiran harian anak Anda selama pekan ini."`. Preserved `data-testid="attendance-week-summary"` + `"Minggu ini"` literal to keep `e2e/parent.spec.ts:68-69` green. `npm run build` ✓, `npx vitest run` 273 pass / 42 todo, `npx eslint app/parent/attendance/` clean.
- **Task S5a (SummaryHero primitive):** New `components/portal/summary-hero.tsx` + `components/portal/__tests__/summary-hero.test.tsx` (19 tests, all pass). Tone-tinted hero card with 5 tones (danger / warn / success / celebration / neutral) — token-only classing (`bg-destructive/8`, `bg-status-late-subtle`, `bg-status-present-subtle`, `bg-celebration-gold-subtle`, `bg-card`) + 4 px tone-left-accent (`border-l-4` + tone-colored `border-l-*`), `rounded-xl`, `p-card` spacing token (no raw `p-5`/`p-6`), `shadow-card-elevated` default with `elevated={false}` → `shadow-card-resting` for secondary placements. Text ramp: `text-display font-bold tracking-tight` on primary, `text-sm text-muted-foreground mt-1` on secondary. Icon slot `size-6` stroke-2 in tone-strong color. Layout: column on mobile (action full-width below), row on ≥sm (action right-aligned). Accessibility: `<section role="region">` + `aria-hidden` icon; action slot preserves its own keyboard behavior (no wrapper interception). No self-animation — consumers own page-level motion (prevents double-animations when wrapped). **Extraction rationale — 2nd-instance trigger met per `.claude/standards/portal.md` §Portal Primitive Inventory ("The 2nd instance is the extraction trigger"):** 4 consumers — T1 parent-home urgency banner (attention + all-clear Alhamdulillah), T2 parent-invoices outstanding money hero, T3 parent-attendance weekly Hadir/Sakit/Alpa/Izin summary, T4 parent-reports latest-rapor celebration. Cross-checked `.claude/standards/design-system.html` §14 Page Recipes (Household Overview urgency banner + Parent money hero patterns). `npx vitest run` 273/273 pass, `npm run lint` 0 new errors. No consumer wired yet — T-tasks do the swap. (Pre-existing TS error in `app/api/admin/seed/route.ts:426` unrelated to this task.)
- **Task T5 (parent student-journal polish):** `components/portal/week-grid.tsx` — polish-only, no structural rework. (T5a) Today-column tint strengthened: header cell `bg-primary/10` → `bg-status-present-subtle` + `text-primary font-semibold` + `border-t-2 border-primary`; body cells `bg-primary/5` → `bg-status-present-subtle`; final indicator row of the last category receives `border-b-2 border-primary` on the today cell so the teal bracket closes cleanly (single-column accent). (T5b) Jumat-no-clip at 375 px: sticky label column narrowed `w-[120px] min-w-[120px]` → `w-[104px] min-w-[104px]`; table `min-w-[340px]` → `min-w-[324px]`. 104 + 5×44 = 324 px fits within the 375 px viewport minus the 2×16 px parent padding and the inner `-mx-4 px-4` gutter — Jumat visible without horizontal scroll. Sticky-column behavior preserved. (T5c) Category headers (Ibadah / Akademik / Karakter) warmed: `bg-muted/40 px-2 py-1.5 text-xs` → `border-l-4 border-l-primary bg-primary/5 pl-3 py-2 text-h2 font-semibold` per design-system.html §15 Student Journal mockup. (T5d) NoteThread timestamps audited — `components/student-journal/note-thread.tsx` + `app/parent/student-journal/page.tsx` already use `formatDateShort` from `@/lib/format`; zero `toLocale*` usages. Already clean. WeekGrid contract unchanged (editable / readonly / home-note modes all work). Cross-consumer impact check: teacher + admin student-journal pages re-render fine. Gates: `npm run build` ✓, `npm run lint` 0 errors, `npx vitest run components/portal components/student-journal` 45/45 pass. (Pre-existing `app/parent/invoices/__tests__/client.test.tsx` failures are from in-progress sibling tasks — confirmed unrelated to T5 by re-running with week-grid.tsx stashed; same failures.)
- **Task T1b/c/d (HouseholdOverview rewrite — SummaryHero + CardListItem visual + intent chips + celebration):** `components/parent/household-overview.tsx` rewritten (+215 / −133 lines, internal only; `HouseholdChild` type surface preserved). **T1b:** urgency banner replaced with `SummaryHero` — `tone="danger"` + `AlertCircle` icon when any kid needs attention, `tone="celebration"` + `Sparkles` icon when all-clear (uses S4 gold token triad via SummaryHero built-in mapping). Single elevated primary surface, principle 1 addressed. **T1c:** `ChildRow` → `ChildCard` — whole row is one `<Link>` (nesting a CardListItem Link inside another Link is invalid HTML) that visually matches CardListItem tokens (rounded-xl, border, bg-card, hover `bg-muted/50`, `active:scale-[0.98]` press state, focus-visible ring, `shadow-card-resting`). Avatar `size-11` with brand-teal gradient `from-primary/25 to-primary/5`. Name uses full width on `text-base font-semibold` with no truncate (was clipping at ~14 chars). Today chip now `StatusBadge variant="intent"` — icon + label + 4 px left accent, decipherable at 3 m glance (principle 4). **T1d:** `SignalCell` reworked — each of 3 cells leads with a Lucide icon (`Wallet`/`Calendar`/`Sparkles`/`BookOpen`), uppercase micro-label, value, optional hint. Paid state uses `bg-status-present-subtle` + `text-status-present-text` (warm green, not grey). Published rapor uses `bg-celebration-gold-subtle` + `text-celebration-gold-text` — the celebration warmth ships. Tokens only — zero raw hex. Cross-check: `.claude/standards/design-system.html` §14 Household Overview page recipe + §Status-chip set (intent + tone-tinted left accent); `.claude/standards/portal.md` §Household Overview + §Portal Primitive Inventory. Lint clean; local TS check clean on touched files. Build + vitest are RED from sibling targets' WIP outside T1 scope (documented in T1a).
- **Task T1e (2-kid fallback polish):** `app/parent/page.tsx` <3-kid branch — student Card upgraded to match T1 craft level: `p-5` → `p-card` token, `shadow-card-elevated` added (S4), avatar inner circle `bg-primary/10` → `bg-gradient-to-br from-primary/25 to-primary/5` (brand-teal gradient matching ChildCard), class `StatusBadge` now uses `variant="intent"` (icon + left accent). Name / nickname / program-name all `min-w-0` + `truncate`-safe for long Indonesian names on 375 px. Greeting already upgraded in T1a; QuickLinkCard trio left untouched (QuickLinkCard is outside T1 file scope but the surrounding card now looks cared-for). Cross-check: `.claude/standards/design-system.html` §14 Page Recipes + §Status-chip set (intent variant). Lint clean.
- **Task T1a (parent-home greeting + Islamic motif):** New `components/parent/parent-greeting.tsx` replaces `PageHeader` on `/parent` in both the ≥3-kid and <3-kid branches (`app/parent/page.tsx`). Heading now uses `text-display font-bold tracking-tight text-foreground` ramp (design-system.html §3 scale — cycle-2 used `text-2xl` which flattened the hierarchy per cycle doc Principle 2 diagnosis). Inline SVG rub-el-hizb motif (4×4 dot-lattice + 8-pointed star overlay, <600 bytes, no external asset) sits absolute-right at `opacity: var(--motif-opacity)` (S4 token, 3%) inheriting `text-primary` via `currentColor`. `pointer-events-none` + `aria-hidden="true"` — invisible to AT. Cross-check: `.claude/standards/design-system.html` §14 Page Recipes + §17 Voice; `.claude/standards/portal.md` §Household Overview (Islamic courtesy layer, Principle 7). Lint 0 errors. Build + some vitest are RED due to parallel sibling targets' WIP on `components/portal/week-grid.tsx` and `app/parent/invoices/**` — both outside T1 scope per cycle spec; controller will sequence merges.

- **Task T2a/T2b/T2c (parent invoices list visual overhaul):** `app/parent/invoices/client.tsx` rewritten (216 → 291 lines). Replaces DataTable with `SummaryHero` + CardListItem list per design-system.html §14 Page Recipes (Parent money hero pattern) + portal.md §Portal Primitive Inventory "<10 items → card list" rule. (1) **T2a SummaryHero** — placed before PageHeader: `tone="danger"` + `Wallet` icon + `formatRupiah(totalOutstanding)` primary + `"{count} tagihan · jatuh tempo {formatDateShort(nearestDue)}"` secondary when outstanding > 0; `tone="celebration"` + `Sparkles` icon + `"Alhamdulillah, semua lunas."` primary when zero. Totals + nearest-due computed client-side from existing data — no API change. (2) **T2b CardListItem list** — `<ul>` of `<CardListItem onClick>` rows; leading = `size-10 rounded-full bg-primary/10` circle with `Receipt` Lucide icon in primary teal; primary = invoice period; secondary = `{invoiceNumber} · Jatuh tempo {formatDateShort(dueDate)}`; trailing stacks `<StatusBadge variant="intent">` chip + `font-currency tabular-nums` amount. Newest-first via `periodLabel.localeCompare`. (3) **T2c empty states** — `EmptyState accent="celebration"` + Sparkles on "Belum Bayar"/"Jatuh Tempo" filters with zero matches (Alhamdulillah copy); `accent="warm"` + Receipt on "Lunas" filter with zero matches ("Belum ada pembayaran"); `accent="warm"` + Receipt on "Semua" empty. (4) **Status-color correctness** — new `chipStatusFor(status)` helper remaps `SENT → OVERDUE` on the chip only (underlying invoice status untouched for API/filter use). Cycle-2 bug: UNPAID/SENT rendered blue (`status-leave-subtle`), colliding with IZIN/permission semantics. Fix uses S1 intent variant with `status="OVERDUE"` → red (`status-absent` family + `border-l-destructive`) + `AlertTriangle` auto-icon, per design-system.html §Status palette (unpaid = danger-severity). Test suite `app/parent/invoices/__tests__/client.test.tsx` rewritten (26 tests, all pass): SummaryHero tone assertions, CardListItem rendering + onClick, per-filter empty state verification, S1 intent chip copy, DataTable-absence assertion. `npm run build` ✓, `npx vitest run` 273 pass / 42 todo, `npm run lint` 0 new errors. Cross-checked `.claude/standards/design-system.html` §14 Page Recipes + §Status palette, `.claude/standards/portal.md` §Portal Primitive Inventory + Data-display rule, `.claude/standards/voice.md` parent empty-state + Tagihan glossary.
- **Task T2d (parent invoice detail sheet visual pass):** `app/parent/invoices/invoice-detail-sheet.tsx` rewritten (307 → 348 lines). API contract preserved: same props, same fetch, same open/close logic — visual-only pass. (1) **SummaryHero header** — replaces the old flat `p-3 rounded-lg` status banner. Tone mapping: PAID → `success` (CheckCircle2 icon, totalDue + "Lunas pada {paidAt}" secondary); PARTIALLY_PAID → `warn` (CircleDot icon, `remaining` as primary); OVERDUE → `danger` (AlertTriangle icon, remaining + jatuh-tempo secondary); SENT → `danger` (Wallet icon); CANCELLED → `neutral` (MinusCircle icon). `elevated={false}` — secondary hero inside an already-elevated Sheet surface, per SummaryHero JSDoc ("secondary placement"). Primary number uses `text-display` via SummaryHero internal; tabular-nums preserved. (2) **Status chip** — Sheet header chip switched to `variant="intent"` with `sheetChipStatusFor()` mirror of the list-page remap (SENT → OVERDUE, red status-absent family + AlertTriangle icon); consistent color story with the list. (3) **Line items tabular-nums** — amounts gain `font-currency text-sm font-semibold tabular-nums` for column alignment; labels `text-sm font-medium`. `<div>` list → semantic `<ul><li>`. (4) **Payment history as mini CardListItems** — each payment row is a bordered rounded-lg card with method-icon circle (Banknote / Landmark / CreditCard via METHOD_ICONS map) in `bg-status-present-subtle`, method label primary, date + reference secondary, tabular amount trailing in `text-status-present-text`. Much more scannable than the cycle-2 inline flex row. (5) **CTA polish** — "Bayar Sekarang" kept as primary `size="lg"` full-width Button with ExternalLink icon; fallback hint when payment link not yet provisioned moved out of the hero into an explicit muted info card below summary — the hero stays focused on the number. (6) **Padding tokens preserved** — body wrapper keeps `px-card pb-card space-y-field` (cycle-2 G1 overlay inner-padding rule); no double-padding. Gates: `npm run build` ✓, `npx vitest run` 273 pass / 42 todo (unchanged — sheet is rendered only after dynamic import, no test regression), `npm run lint` 0 new errors. Cross-checked `.claude/standards/design-system.html` §13 Overlays (sheet padding) + §14 invoice detail pattern, `.claude/standards/voice.md` (warm error copy preserved on fetch failure).
- **Task T2e (visual verification):** Playwright screenshots deferred to controller for final visual bundle. Local dev server returned 500 during capture attempt; parallel T1/T3/T4 landings touching /parent are racing through the same worktree, so a stable production-build smoke belongs at the controller stage after all targets are merged. T2a/b/c/d changes are verified by unit tests (26 pass for list, sheet only loaded via dynamic import + covered by existing guardian integration tests upstream). All shared primitives consumed (SummaryHero, CardListItem, StatusBadge variant="intent", EmptyState accent) have their own passing unit suites (19 + 6 + 5 + 7 tests respectively). No token drift, no raw hex, no arbitrary sizes — primitives do the heavy lifting. Between-task gates green at every commit.
- **Task T4 (parent/reports celebration overhaul):** `app/parent/assessments-table.tsx` rewritten to consume cycle-3 shared primitives (`SummaryHero`, `CardListItem`, extended `EmptyState`, `StatusBadge variant="intent"`); `app/parent/reports/page.tsx` passes the selected child's nickname/first-name for warm voice. **T4a** — latest-rapor `SummaryHero` above the list: `tone="celebration"` + `Sparkles` icon + `elevated={true}`; primary "Rapor {period} {nickname} sudah terbit" and secondary "Alhamdulillah — {programName}. Ketuk untuk baca penilaian lengkap Ustadzah." with a primary `Button size="sm"` action "Lihat Rapor" that triggers the existing sheet handler. All rows returned by `getPublishedAssessmentsForStudent` are `PUBLISHED` (helper already filters), so the DRAFT-muted hero branch collapses into T4d: when there is no rapor at all the hero is skipped per spec and a warm empty state is shown. **T4b** — per-semester rows rendered as `<CardListItem>`: `size-11 rounded-full bg-primary/10 text-primary` leading with a `<GraduationCap>` icon, primary `templateName`, secondary `period · programName`, trailing `<StatusBadge status="PUBLISHED" variant="intent" />` (Sparkles + green-subtle per cycle-3 S1 intent map); `onClick` opens the existing detail Sheet — the full-width "Lihat" button column is gone, replaced by the card's own press-state. **T4c** — detail sheet visual pass: secondary `SummaryHero` inside the sheet (`elevated={false}`, `tone="celebration"`) under the existing SheetHeader; category headings promoted from uppercase-muted micro-label to `text-h2 font-semibold text-foreground`; score badges gain `tabular-nums` for column alignment; new **Catatan Ustadzah** block at the end of the sheet renders any per-indicator `notes` in a warm-accent card (`border-l-4 border-l-primary bg-primary/5 p-card rounded-md`). `p-card` inner wrapper and overlay padding tokens preserved (cycle-2 G1 rule). No new API or Prisma changes — `publishedAt` / `teacherName` are not in the helper payload and were deliberately omitted rather than fetched, per cycle restrictions; the hero and GraduationCap leading icon carry the warmth instead. **T4d** — no-rapor empty state upgraded to `<EmptyState accent="warm" icon={BookOpen}>` with "Rapor belum terbit" + "InsyaAllah akan tersedia setelah Ustadzah finalisasi nilai. Periksa kembali di akhir semester." copy per voice.md rapor patterns. Gates: `npm run lint` 0 errors on touched files (18 pre-existing warnings elsewhere), `npx vitest run` all reports-scope tests pass (11 pre-existing fails are in T2's `app/parent/invoices/__tests__/client.test.tsx`, unrelated), `npx tsc --noEmit` clean on touched files (pre-existing T1 error in `app/parent/page.tsx:243` — second `<PageHeader>` reference not yet swapped by T1, out of T4 scope). Cross-checked `.claude/standards/design-system.html` §14 Page Recipes (rapor celebration pattern), `.claude/standards/voice.md` (rapor publish = celebration event; "Alhamdulillah · InsyaAllah" patterns). T4e Playwright screenshots deferred to controller — pre-existing T1 build error blocks a full production-server smoke.
