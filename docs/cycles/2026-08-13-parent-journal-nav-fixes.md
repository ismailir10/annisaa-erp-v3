...
- Preview-verify iteration 1 (`https://annisaa-erp-v3-git-feat-parent-013737-ismails-projects-196d40d3.vercel.app`, PR #492): flows=[parent Jurnal Di Sekolah/Di Rumah, week navigation across the window boundary, Lainnya overflow sheet], **blockers=0, minors=0**. Signed in via Chrome MCP as the parent account from `.claude/verify-accounts.json` (guardian "Nurul", children Bilal Hakim + Hafizh Umar Ramadhan). Observed:
  - **The blocker fix is live.** Week 10–14 Aug renders the Di Sekolah / Di Rumah / Catatan tabs with every cell empty (`—`). Under the old gate this exact state produced the dead-end "Belum ada catatan minggu ini" and nothing was tappable.
  - **Window boundary is exact.** Current week: 08/10–08/13 editable, 08/14 locked as `"tanggal akan datang belum bisa diubah"`. Previous week: 08/03–08/07 all editable — 08/03 is precisely the floor. Two weeks back: 07/27–07/31 all locked as `"di luar jangkauan — hanya bisa diubah dari Senin minggu lalu sampai hari ini"`. Floor anchoring confirmed empirically: navigating backwards never unlocked a cell.
  - **Past-day write succeeds.** Ticking Mon 08/10 (impossible before this cycle) → `POST /api/student-journal/entries/home` **200**, followed by the week refetch **200**; the check persisted and rendered.
  - **T5 copy:** Lainnya sheet reads "Perkembangan / Perkembangan anak per elemen" — no "Capaian" anywhere in nav copy.
  - Zero console errors, no 4xx/5xx, no layout breakage at 1453×840.
  - Fixtures left in place per the seed-via-CRUD rule (non-destructive cycle): two HOME journal ticks on Bilal Hakim in the staging DB, dated 2026-08-10 and 2026-08-11.
