...
- Task 1: `npm run build` exit 0; `npx vitest run` (clean env) → 379 passed, 2 skipped files / 3555 tests passed. Note: running vitest with `DEMO_MODE`/`DATABASE_URL` exported in the shell fails 19 xendit/auth-rate-limit tests — an env leak from the build shell, not a regression; gates run with those unset. `tsc --noEmit` clean; eslint 0 errors. design-system: tabs keep the `bg-muted` pill variant, sticky column uses `bg-background`/`bg-muted` tokens only.
