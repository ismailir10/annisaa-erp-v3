...
| **students** | Student lifecycle — auth-proxied photo upload, guardians (one parent shared across siblings; Tambah Wali searches first, warns on duplicate name, phone, NIK or email; the active wali utama, set in one click, is billed), enrollments (one school plus one day-care placement per year; age bands advisory, audited override), admissions (`/daftar` inquiry → token → form → convert), CSV export. Detail is a single-scroll dossier — hash-addressable sections, NIK/KK masked, read-only Keuangan / Keringanan / Jurnal / Akademik / Pendaftaran, rail tiles from one aggregate route |
...
Role homes apply “Don't Make Me Think”: teachers see their class and unfinished work, guardians see family actions and clearly identified children, and admins get a one-screen Dasbor (queue counts, the five most urgent items, today's staff attendance) with the full list at `/admin/work-queue`. The admin sidebar holds daily work only; setup (campuses, academic years, semesters, holidays, fees, salary components, templates, users and roles) lives in the `/admin/settings` hub. Shared shadcn components keep navigation, context, save feedback, and recovery consistent; the two mobile portals retain five navigation slots.
