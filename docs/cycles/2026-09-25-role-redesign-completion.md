...
- Signed-in preview-verify passed for runtime source `563b992d9fe311f44b475c7d5c0dc02dd3578f16` at `https://annisaa-erp-v3-a3q85xmor-ismails-projects-196d40d3.vercel.app` (Vercel deployment `dpl_DjHWXuvd12Eq7bFUtAxzZ44Lbox9`, READY and source SHA verified through Vercel). Normal Google OAuth used each authorized role account. Four focused flows: admin queue → correct enrollment/leave/payroll records, teacher home → assigned attendance/journal with class/date reload, parent sibling card → child-specific notes with reload, and parent bills → invoice detail/forged paid callback. Three role homes and mobile teacher/journal/admin screens were visually inspected; the screenshot evidence remains private in the task because shared preview records may contain personal data. Public committed screenshots use only disposable local fixtures.
  - Teacher assignment, attendance, and class-grid requests returned 200. Admin leave/stats/payroll/compare requests returned 200. Parent invoice/detail requests returned 200; an existing preview test invoice remained server `SENT` with zero paid after `paymentStatus=paid`, and the callback parameters were removed while the validated child stayed selected. No payment or approval was submitted. Saved-state mutation/retry and restricted-role/ownership boundaries were exercised locally as recorded below; preview verification was read-only apart from normal authentication/read markers.
  - No console errors/warnings, observed workflow HTTP errors, blank states, or unexpected sign-in loops. Mobile viewport override was reset. Browser timing/selector tool errors were recovered by observing the actual rendered state; they were not application failures. Preview evidence source remains distinct from later test-only or evidence-only commits.
- First protected CI run on `80e3effc`: Docs sync, Lint/Typecheck/Test, and standard Build passed. Playwright reported one locale-specific assertion failure: Ubuntu Chromium renders Indonesian clock times with dot separators, while the test required colons. Application state and saved timestamps were correct. The test now compares exact saved timestamps using the browser’s locale; runtime behavior, skip counts, and timeouts are unchanged. Six focused teacher E2E tests and scoped ESLint passed; latest-head CI must pass before merge.
...
- The design reference iframe was blocked by a duplicate proxy `X-Frame-Options: DENY` override. A reviewed exact-path exception allows only `/admin/design-system-reference.html` to use SAMEORIGIN with matching report-only frame ancestors; every other path retains DENY. Rebuilt source `59254ff4` passed both reference E2E tests, including actual iframe content and exact-path header isolation; E2E guards loaded content rather than merely the iframe element.
...
- First full Playwright run: **136 passed, 14 failed, one flaky, eight skipped**. Failure review identified stale home/queue assertions, removed native invoice list semantics, a real class/year mismatch in the billing wizard, and a local pickup fixture that inadvertently replaced the active academic year. Stale assertions, native list semantics, billing scope, and exact disposable fixture-year state were corrected; the subsequent full run passed as recorded above. This first run remains historical failure evidence.
