...
| Date | Decision | Why |
|---|---|---|
| 2026-05-18 | Kesiswaan CRUD parity (in flight): superset parent-option constants; auth-proxied Student photo upload (≤2 MB JPG/PNG, magic-byte validated, never under `public/`); Student metadata key/value editor; "Riwayat Status" sub-card surfaces lifecycle dates + inline withdrawalReason edit | KTP/KK (T14) needs the storage adapter; metadata grid was read-only; lifecycle fields were saved but never displayed — see [cycle](docs/cycles/2026-05-18-kesiswaan-crud-audit.md) |
