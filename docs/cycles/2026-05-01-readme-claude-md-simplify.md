...
- Task 6 review-fix: code-reviewer agent flagged 1 BLOCKER + 1 MAJOR. (BLOCKER) `case " $STAGED " in *' README.md '*` failed to match newline-separated `$STAGED`, so Rule 6 silently skipped any commit staging README.md alongside other files (the normal case). Replaced with `while IFS= read -r f` loop matching the rest of the hook. New ADR4 test case stages README.md + a cycle doc with a 500-char ADR cell — locks the regression in. test-hooks.sh now 21/21 green. (MAJOR) README.md ADR row dated `2026-04` linked to `docs/cycles/2025-04-15-...md` — date mismatched the linked cycle's "Cycle Date: 2025-04-15". Fixed row date to `2025-04`.
- Task 6: end-of-cycle gates green (build=0, vitest=0, test-hooks 21/21). **Playwright skipped** — pure-docs cycle, zero UI risk; recorded explicitly here per spec acceptance. Final invariant sweep:
